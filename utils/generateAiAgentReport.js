...
function generateMD(data) {
  ...
  lines.push('|--------|-------|----------|---------|--------|------|----------|');
  for (const [module, info] of Object.entries(data.moduleSummary).sort((a, b) => b[1].total - a[1].total)) {
  ...
  lines.push('');
  const agentsWithDuration = data.agents.filter(a => a.duration > 0);
  if (agentsWithDuration.length > 0) {
    const totalDur = agentsWithDuration.reduce((sum, a) => sum + a.duration, 0);
    const avgDur = Math.round(totalDur / agentsWithDuration.length);
    const maxDur = Math.max(...agentsWithDuration.map(a => a.duration));
    const minDur = Math.min(...agentsWithDuration.map(a => a.duration));
    ...
    lines.push('|-------|----------|');
    agentsWithDuration.sort((a, b) => b.duration - a.duration).forEach(a => {
...
<div class="container">
  ...
  <div class="section">
    ...
    <table>
      ...
      <tbody>
        ${data.environment ? Object.entries(data.environment).map(([k, v]) =>
...
function renderModules() {
  ...
  const entries = Object.entries(moduleData).sort((a, b) => b[1].total - a[1].total);
  const totalAll = Object.values(moduleData).reduce((s, m) => s + m.total, 0);
...
module.exports = { main, generateHTML, generateJSON, generateMD, generateCSV, generatePDF };
