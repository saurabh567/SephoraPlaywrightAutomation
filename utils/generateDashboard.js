    ...
    lines.forEach(line => {
      if (line.startsWith('|') && line.endsWith('|')) {
        ...
        if (!inTable) { headers = cells; inTable = true; }
          ...
          const suggestion = {};
          headers.forEach((h, i) => { suggestion[h] = cells[i] || ''; });
...
function collectFlakyData() {
  ...
  const flakyScenarios = Object.entries(scenarioMap)
    .filter(([, statuses]) => {
...
function generateDashboardData() {
  ...
  const data = {
    ...
    mobileDashboard: {
      android: {
        scenariosRun: cucumberData ? cucumberData.scenarioDetails.filter(s => s.tags && s.tags.includes('@android')).length : 0,
        passed: cucumberData ? cucumberData.scenarioDetails.filter(s => s.tags && s.tags.includes('@android') && s.status === 'passed').length : 0,
        failed: cucumberData ? cucumberData.scenarioDetails.filter(s => s.tags && s.tags.includes('@android') && s.status === 'failed').length : 0,
        screenshots: mobileData.android.screenshots.length,
        videos: mobileData.android.videos.length,
      },
      ios: {
        scenariosRun: cucumberData ? cucumberData.scenarioDetails.filter(s => s.tags && s.tags.includes('@ios')).length : 0,
        passed: cucumberData ? cucumberData.scenarioDetails.filter(s => s.tags && s.tags.includes('@ios') && s.status === 'passed').length : 0,
        failed: cucumberData ? cucumberData.scenarioDetails.filter(s => s.tags && s.tags.includes('@ios') && s.status === 'failed').length : 0,
...
${pr.maturedCapabilities.map(c => `- ${c.name}: ${c.status}`).join('\n')}
...
function generateHTML(data) {
  ...
  const failedScenarios = func.cucumber.failedScenarios && func.cucumber.failedScenarios.length > 0 ?
    func.cucumber.failedScenarios.map(fs => `
    <div class="feature-item">
      ...
      ${fs.steps && fs.steps.length > 0 ? `<div style="margin-top:6px;font-size:12px">
        ${fs.steps.filter(s => s.status === 'failed').map(s => `<div style="color:var(--accent-red)">✗ ${s.keyword}${s.name}</div>`).join('')}
      </div>` : ''}
    </div>`).join('\n') : 
  ...
  const perfChartLabels = perf.topSlowAPIs && perf.topSlowAPIs.length > 0 ? perf.topSlowAPIs.map(a => a.label.substring(0, 20)) : ['No data'];
  const perfChartValues = perf.topSlowAPIs && perf.topSlowAPIs.length > 0 ? perf.topSlowAPIs.map(a => a.avgResponse) : [0];
  ...
  const slowAPIsData = JSON.stringify(perf.topSlowAPIs && perf.topSlowAPIs.length > 0 ? 
    perf.topSlowAPIs.map(a => ({ label: a.label.substring(0, 25), value: a.avgResponse })) : 
  ...
  <div class="container">
    ...
    <div class="section">
      ...
      <div class="card animate-in">
        ...
        <table class="data-table">
          ...
          <tbody>
            ${sh.suggestions.slice(0, 5).map(s => `
            <tr>
              <td>${s.Problem || 'N/A'}</td>
    ...
    <div class="section">
      ...
      <div class="cards-grid">
        <div class="card animate-in">
          ...
          ${flaky.flakyScenarios && flaky.flakyScenarios.length > 0 ? flaky.flakyScenarios.slice(0, 10).map(fs => `
          <div class="feature-item">
            <div class="feature-header">
              <span class="feature-name">⚠ ${fs.name}</span>
            </div>
            <div class="feature-stats" style="margin-top:4px">
              ${fs.runs.map(r => `<span class="badge ${r.status === 'passed' ? 'badge-pass' : 'badge-fail'}">${r.status}</span>`).join(' ')}
            </div>
    ...
    <div class="section">
      ...
      <div class="release-card ${releaseCardClass} animate-in">
        ...
        ${rg.reasoning.map(r => `<div class="release-reason">${r}</div>`).join('')}
      </div>
    </div>
