...
function loadData() {
  const lifecycleData = fs.existsSync(LIFECYCLE_STORE)
    ...
    : { agents: {}, updatedAt: new Date().toISOString(), executionId: null };
...
function analyzeAgents(lifecycleData, execContext) {
  const agents = Object.values(lifecycleData.agents || {});
  const byState = {};
  const byStage = {};
  const byHealth = {};
  const failures = [];
  ...
  for (const agent of agents) {
    ...
    if (agent.totalRuns > 1 || agent.stateHistory.filter(h => h.from === 'FAILED' || h.to === 'RUNNING').length > 1) {
      retries.push(agent);
    }
  ...
  for (const agent of agents) {
    if (agent.dependencies && agent.dependencies.length > 0) {
      depChains[agent.key] = agent.dependencies.map(d => {
...
function generateJSON(analysis) {
  const report = {
    ...
    failureChain: analysis.failures.map(a => ({
      key: a.key,
      consecutiveFailures: a.consecutiveFailures,
      lastError: a.lastError,
      lastFailedAt: a.lastFailedAt,
    ...
    recoveryChain: analysis.recoveries.map(a => ({
      key: a.key,
      recoveryCount: a.recoveryCount,
      lastRecoveredAt: a.lastRecoveredAt
    })),
    retryChain: analysis.retries.map(a => ({
      key: a.key,
      totalRuns: a.totalRuns,
      successCount: a.successCount,
      failureCount: a.failureCount
    })),
    skippedAgents: analysis.skipped.map(a => ({
      key: a.key,
      skipCount: a.skipCount,
      lastError: a.lastError,
      lastSkippedAt: a.lastSkippedAt
    ...
    executionOrder: analysis.timeline
      .filter(t => t.to === 'RUNNING' || t.to === 'COMPLETED' || t.to === 'FAILED')
      .map(t => ({ agent: t.agent, name: t.name, action: t.to, at: t.at, duration: t.duration }))
  };
...
function generateMD(analysis, jsonReport) {
  const lines = [];
  ...
  lines.push('|-------|-------|');
  for (const [state, count] of Object.entries(analysis.byState).sort((a, b) => b[1] - a[1])) {
    lines.push('| ' + state + ' | ' + count + ' |');
  }
  lines.push('');
  ...
  lines.push('|--------|-------|');
  for (const [health, count] of Object.entries(analysis.byHealth).sort((a, b) => b[1] - a[1])) {
    lines.push('| ' + health + ' | ' + count + ' |');
  }
  lines.push('');
  ...
  lines.push('|-------|-------|');
  for (const [stage, count] of Object.entries(analysis.byStage).sort((a, b) => b[1] - a[1])) {
    lines.push('| ' + stage + ' | ' + count + ' |');
  }
  lines.push('');
  ...
  lines.push('');
  const execOrder = analysis.timeline.filter(t => t.to === 'RUNNING' || t.to === 'COMPLETED');
  if (execOrder.length > 0) {
    lines.push('| # | Agent | Stage | Status | Duration |');
    lines.push('|---|-------|-------|--------|----------|');
    execOrder.forEach((t, i) => {
  ...
  lines.push('');
  const deps = Object.entries(analysis.depChains);
  if (deps.length > 0) {
    for (const [agent, dependencies] of deps) {
      const depList = dependencies.map(d => d.key + ' (' + d.state + ')').join(', ');
  ...
  lines.push('');
  const withDuration = analysis.agents.filter(a => a.lastDuration > 0);
  if (withDuration.length > 0) {
    const total = withDuration.reduce((s, a) => s + a.lastDuration, 0);
    const avg = Math.round(total / withDuration.length);
    const max = Math.max(...withDuration.map(a => a.lastDuration));
    lines.push('| Metric | Value |');
    lines.push('|--------|-------|');
    ...
    lines.push('|-------|----------|');
    withDuration.sort((a, b) => b.lastDuration - a.lastDuration).forEach(a => {
...
function generateHTML(analysis, jsonReport) {
...
(function() {
  ...
  const maxVal = Math.max(...Object.values(healthCounts), 1);
...
(function() {
  ...
  const entries = Object.entries(depChains);
...
async function main() {
  ...
  if (agentCount === 0) {
    ...
    const empty = { reportType: 'agent-lifecycle', generatedAt: new Date().toISOString(),
      summary: { totalAgents: 0, stateDistribution: {}, stageDistribution: {}, healthDistribution: {} },
      agents: [], failureChain: [], recoveryChain: [], retryChain: [],
      skippedAgents: [], executionTimeline: [], dependencyChains: {}, executionOrder: [] };
