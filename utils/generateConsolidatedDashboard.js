...
function findSuiteStatus(statusArr, suiteName) {
...
function readLifecycleSummary(filePath) {
  const content = safeReadFile(filePath);
...
function collectWebData() {
  ...
  (data || []).forEach(feature => {
    ...
    (feature.elements || []).forEach(sc => {
      const steps = sc.steps || [];
      const hasFail = steps.some(s => s.result && s.result.status === 'failed');
      const allPass = steps.every(s => s.result && s.result.status === 'passed');
      const status = hasFail ? 'failed' : allPass ? 'passed' : 'skipped';
      const dur = steps.reduce((s, step) => s + ((step.result && step.result.duration) || 0), 0);

      total++;
      totalDurationMs += dur;
      if (status === 'passed') { passed++; fPassed++; }
      else if (status === 'failed') { failed++; fFailed++; failedScenarios.push({ feature: fName, name: sc.name, status, duration: Math.round(dur / 1e6), tags: (sc.tags || []).map(t => t.name || t) }); }
...
function collectAndroidData(execStatus, lifecycleSummary) {
...
function collectiOSData(execStatus, lifecycleSummary) {
...
function collectAPIData() {
  ...
  if (!summary && !apiCucumber) {
    return { available: false, status: 'Not executed', results: [], summary: { total: 0, passed: 0, failed: 0, passRate: 0 } };
  }

  const results = (summary && summary.results) || [];
  const total = (summary && summary.totalAPIs) || (results.length) || 0;
  const passed = (summary && summary.passed) || results.filter(r => r.passed).length || 0;
  const failed = (summary && summary.failed) || results.filter(r => !r.passed).length || 0;
...
function collectSelfHealingData() {
  ...
  const healed = locatorEntries.filter(e => e.healed || e.autoApplied || (e.status && e.status === 'healed')).length;
...
function collectFlakyData(webData) {
  ...
  if (webData && webData.available && webData.scenarioDetails) {
    const seen = {};
    webData.scenarioDetails.forEach(sc => {
      if (seen[sc.name]) {
...
function collectRootCauseData(webData, apiData, androidData, iosData) {
  const categories = {
    locator: 0, assertion: 0, timeout: 0, testData: 0,
    environment: 0, api: 0, mobileDevice: 0, performance: 0, unknown: 0,
  };

  const webFailed = webData && webData.failedScenarios ? webData.failedScenarios : [];
  const apiFailed = apiData && apiData.results ? apiData.results.filter(r => !r.passed) : [];
  const androidFailed = androidData && androidData.failedScenarios ? androidData.failedScenarios : [];
  const iosFailed = iosData && iosData.failedScenarios ? iosData.failedScenarios : [];
...
function generateDashboardData() {
  ...
  allSuites.forEach(s => {
    if (s.data && s.data.available) {
      suitesExecuted++;
      if (s.data.summary) {
        grandTotal += s.data.summary.total || 0;
        grandPassed += s.data.summary.passed || 0;
        grandFailed += s.data.summary.failed || 0;
        grandSkipped += s.data.summary.skipped || 0;
        if (s.data.summary.failed === 0) suitesPassed++;
        else suitesFailed++;
      }
    }
  });
  ...
  score += apiData && apiData.summary && apiData.summary.passRate > 0 ? Math.min(15, parseFloat(apiData.summary.passRate) * 0.15) : 5;
  ...
  const dashboard = {
    ...
    releaseGatekeeper: {
      ...
      selfHealingOk: healingData && healingData.available && healingData.healingSuccess >= 80,
    },
    productionReadiness: {
...
function generateHTML(data) {
  ...
  // API results rows
  function apiResultRows(results) {
  ...
  function getChangedFiles() {
    try {
      ...
      return out.split('\n').map(f => `<div style="font-size:12px;padding:2px 0;color:var(--text-secondary)">📄 ${escHtml(f)}</div>`).join('');
    } catch (_) { return 'Git changes not available'; }
  }
  ...
  const rel = (p) => { try { return path.relative(ROOT, p).replace(/^reports\//, 'reports/'); } catch (_) { return p; } };
  ...
  const webFeatures = w && w.features ? w.features : [];
  const featureNames = JSON.stringify(webFeatures.map(f => f.name));
  const featurePassed = JSON.stringify(webFeatures.map(f => f.passed));
  const featureFailed = JSON.stringify(webFeatures.map(f => f.failed));
  ...
  const androidEnv = a && a.env ? a.env : {};
  const iosEnv = i && i.env ? i.env : {};


  // Helper: Android missing report HTML
  function androidReportMissingHTML(aData) {
    const ei = aData && aData.execInfo ? aData.execInfo : null;
    const rp = aData && aData.reportPaths ? aData.reportPaths : {};
  ...
  function iosReportMissingHTML(iData) {
    const ei = iData && iData.execInfo ? iData.execInfo : null;
    const rp = iData && iData.reportPaths ? iData.reportPaths : {};
  ...
  <div class="section">
    ...
    <div class="card-grid">
      <div class="card anim">
        ...
        ` : perf.summaryData ? `
          ${Object.entries(perf.summaryData).map(([k, v]) => cardRow(k, v)).join('')}
        ` : `
    ...
    <div class="card-grid">
      ...
      <div class="card anim" style="animation-delay:0.08s">
        ...
        <div style="font-size:13px;color:var(--text-secondary);line-height:1.7">
          ...
          ${w && w.features ? w.features.map(f => `<div style="font-size:12px;padding:2px 0">📌 ${escHtml(f.name)}</div>`).join('') : '<div style="font-size:12px;color:var(--text-muted)">No features loaded</div>'}
...
${Object.entries(rc.categories || {}).filter(([_, v]) => v > 0).map(([k, v]) => `- **${k}:** ${v}`).join('\n')}
...
module.exports = { generateDashboardData, generateHTML, generateSummaryMarkdown, main };
