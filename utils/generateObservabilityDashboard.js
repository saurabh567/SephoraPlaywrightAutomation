...
async function main() {
  ...
  const agentDetails = allAgents.map(a => {
    const key = a.key;
    const lc = lifecycleEntries.find(e => e.key === key);
    const perf = perfSummary.agents?.find(p => p.key === key);
    ...
    return {
      ...
      stateHistory: lc ? (lc.stateHistory || []).slice(-20) : [],
      // Perf
  ...
  for (const stage of stageOrder) {
    const agents = agentDetails.filter(a => a.stage === stage);
    stageData[stage] = {
      count: agents.length,
      executed: agents.filter(a => a.totalRuns > 0).length,
      failed: agents.filter(a => a.failureCount > 0).length,
      avgDuration: agents.length > 0
        ? Math.round(agents.reduce((s, a) => s + a.meanDuration, 0) / agents.length)
        : 0,
  ...
  const data = {
    ...
    summary: {
      ...
      totalReachable: allAgents.filter(a => a.metadata.lifecycle === 'active').length,
    ...
    agents: agentDetails,
    perfSummary,
    performanceTrends: perfSummary.agents?.filter(a => a.trend && a.trend !== 'no-data') || [],
...
function render() {
  ...
  const maxState = Math.max(...Object.values(states).filter(v=>typeof v==='number'), 1);
  document.getElementById('stateChart').innerHTML = '<div class="chart-row">' +
    Object.entries(states).filter(([,c]) => c > 0).map(([s,c]) =>
  ...
  document.getElementById('trendTable').innerHTML = '<table><tr><th>Trend</th><th>Count</th></tr>' +
    Object.entries(trendCounts).map(([k, v]) => '<tr><td>' + k + '</td><td>' + v + '</td></tr>').join('') + '</table>';
