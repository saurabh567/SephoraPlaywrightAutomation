...
|------|--------|----------|
${toolResults.map(r => `| ${r.name} | ${r.passed ? '✅' : '❌'} (exit ${r.exitCode}) | ${r.duration} |`).join('\n')}
...
|--------|------|--------|------|
${Object.entries(reportChecks).map(([label, info]) => `| ${label} | \`${info.path}\` | ${info.exists ? '✅ Generated' : '❌ Not found'} | ${info.exists ? formatFileSize(info.size) : 'N/A'} |`).join('\n')}
...
${results.some(r => r.name.includes('iOS')) ? '- iOS Simulator cannot run in true headless mode. This is an Apple Xcode limitation. The simulator window is always visible.' : ''}
