...
function pass(label: any) { results[label] = 'PASS'; }
function fail(label: any) { results[label] = 'FAIL'; }
...
function isPortListening(host: any, port: any) {
...
function checkAppiumHealth(): Promise<any> {
...
function getSimulatorUDID(name: any) {
  try {
...
function getSimulatorState(udid: any) {
  try {
    const out = xcrun(['simctl', 'list', 'devices', '--json']);
    const data = JSON.parse(out);
    const allDevices = data.devices || {};
...
function isSimulatorBooted(udid: any) {
  return getSimulatorState(udid) === 'Booted';
}

function bootSimulator(udid: any) {
  console.log(`[lifecycle] Booting simulator ${udid} ...`);
  xcrun(['simctl', 'boot', udid]);
}

function shutdownSimulator(udid: any) {
...
async function startAppium() {
  ...
  const appium = spawn('npx', ['appium'], {
    detached: true,
    stdio: 'ignore',
    env: { ...process.env, APPIUM_HOST, APPIUM_PORT: String(APPIUM_PORT) },
  });
  appium.unref();
...
async function warmupWDA() {
  ...
  for (const basePath of basePaths) {
    try {
      ...
      try { await warmupDriver.terminateApp("com.apple.mobilesafari"); } catch (_: any) {}
...
async function runIOSTests(): Promise<boolean> {
...
async function stopSimulator(udid: any) {
  if (KEEP_SIMULATOR) {
...
function generateLifecycleReport() {
  ...
  for (const [key, value] of (Object.entries(results) as [string, any][])) {
    const label = key
      .replace(/([A-Z])/g, ' $1')
  ...
  const allPassed = (Object.values(results) as any[]).every((r) => r === 'PASS');
  lines.push('');
...
async function main() {
  ...
  if (udid) {
    try { await stopSimulator(udid); } catch (e: any) {
      console.warn(`[lifecycle] Simulator stop error: ${e.message}`);
    }
  }

  // Step 6 — Stop Appium
  try { await stopAppium(); } catch (e: any) {
