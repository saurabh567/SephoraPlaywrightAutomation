  ...
  try {
    ...
    if (explicitPlatform) {
      ...
      for (const name of platformNames) {
        const platformResult = allResults.find(r => r.platform === name);
        if (!platformResult) {
      ...
      for (const name of platformNames) {
        ...
        if (platformResult && platformResult.reportPaths) {
          for (const [label, info] of Object.entries(platformResult.reportPaths)) {
            if (info.exists) {
      ...
      if (dashboardResult && dashboardResult.reportPaths) {
        for (const [label, info] of Object.entries(dashboardResult.reportPaths)) {
          if (info.exists) {
...
main().catch(async err => {
  ...
  if (aiMonitor) {
    try { await aiMonitor.stop(); } catch (_) {}
  }
  process.exit(2);
});
