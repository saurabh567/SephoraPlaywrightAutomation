...
function collectWebData() {
  ...
  for (const feature of report) {
    const featScenarios: any[] = [];
    for (const el of (feature.elements || [])) {
      total++;
      const status = el.steps.some((s: any) => s.result?.status === 'failed') ? 'failed'
        : el.steps.every((s: any) => s.result?.status === 'passed') ? 'passed' : 'skipped';
      if (status === 'passed') passed++;
      else if (status === 'failed') failed++;
      else skipped++;
      const dur = el.steps.reduce((s: any, st: any) => s + (st.result?.duration || 0), 0);
      durationMs += dur;
...
function collectMobileData(platform: any) {
...
function generateDashboardHTML(platforms: any) {
  const pipelineData = collectPipelineData();

  const platformRows = platforms.map((p: any) => {
...
<div class="container">
  ...
  <div class="execution-order">
    ${['API', 'WEB', 'ANDROID', 'IOS', 'PERFORMANCE', 'AI Analysis', 'Dashboard'].map((step, i) => {
      const p = platforms.find((s: any) => s.name === step || s.name === step.toUpperCase());
  ...
  <div class="platform-data">
    ${['API', 'WEB', 'ANDROID', 'IOS', 'PERFORMANCE'].map(name => {
      const p = platforms.find((s: any) => s.name === name);
...
export { main, generateDashboardHTML, collectWebData, collectAPIData, collectMobileData, collectPerformanceData };
export default { main, generateDashboardHTML, collectWebData, collectAPIData, collectMobileData, collectPerformanceData };
