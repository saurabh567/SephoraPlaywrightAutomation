...
const LocatorApplyManager = require('../ai/agents/LocatorApplyManager');
const gitUtils = require('../ai/tools/gitUtils');
...
function ensureProposalsLoaded(res) {
  if (!res || !res.ok) {
...
async function cmdList() {
  const res = LocatorApplyManager.listProposals();
  ...
  proposals.forEach(p => console.log(formatProposalRow(p)));
}

async function cmdPreview(id) {
  if (!id) return exitWith('--id is required for --preview');
  const res = LocatorApplyManager.generatePatchesFor([id]);
...
async function cmdApply(idsCsv) {
  ...
  const prep = LocatorApplyManager.prepareBackupFor(ids);
  ...
  const applyRes = LocatorApplyManager.applyMultiple(ids);
  if (!applyRes.ok) return exitWith('Apply failed: ' + (applyRes.error || JSON.stringify(applyRes)));

  console.log('Applied proposals:', applyRes.applied.map(a => a.id).join(', '));
...
async function cmdRollback(backupRoot) {
  ...
  const res = LocatorApplyManager.rollback(backupRoot);
