...
async function main() {
  ...
  for (const agent of allAgents) {
    ...
    const perf = performanceSummary.agents?.find(a => a.key === key);
    ...
    agentMap[key] = {
      ...
      stateHistory: lc ? lc.stateHistory || [] : [],
  ...
  const onDemand = agents.filter(a => a.tags.some(t => t.toLowerCase().includes('on-demand') || t.toLowerCase().includes('manual'))).length;
  ...
  lines.push('```');
  const barLen = 30;
  const makeBar = (pct) => '█'.repeat(Math.round(pct / 100 * barLen)) + '░'.repeat(barLen - Math.round(pct / 100 * barLen));
  ...
  lines.push('|----------|-------|----------|---------|--------|------------|');
  for (const [platform, data] of Object.entries(platformBreakdown).sort((a, b) => b[1].total - a[1].total)) {
  ...
  lines.push('|--------|-------|');
  for (const [health, count] of Object.entries(healthCounts).sort((a, b) => b[1] - a[1])) {
    lines.push(`| ${health} | ${count} |`);
  }
  lines.push('');
  ...
  lines.push('|---------------------|------------------|----------|------------|');
  for (const [dep, data] of Object.entries(depChains).sort((a, b) => b[1].total - a[1].total).slice(0, 15)) {
    const covPct = data.total > 0 ? Math.round((data.executed / data.total) * 100) : 0;
  ...
  lines.push('|-------|-------------|');
  for (const [trend, count] of Object.entries(trendCounts).sort((a, b) => b[1] - a[1])) {
