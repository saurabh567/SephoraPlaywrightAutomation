...
let androidCleanupStartTime: any = null;

let iosCleanupStatus = 'NOT_RUN';
let iosCleanupMessage = '';
let iosCleanupStartTime: any = null;
...
function cleanupAndroidEnvironment() {
  ...
  const existingCleanup = trackingLog.find((e: any) => e.suiteName === 'Android Cleanup');
  if (existingCleanup) {
...
function cleanupIOSEnvironment() {
  ...
  const existingCleanup = trackingLog.find((e: any) => e.suiteName === 'iOS Cleanup');
  if (existingCleanup) {
...
function generateIOSFallbackReport() {
  const iosEntry = trackingLog.find(function(e: any) { return e.suiteName === 'iOS Automation'; });
  if (!iosEntry) return;
...
function generateAndroidFallbackReport() {
  var androidEntry = trackingLog.find(function(e: any) { return e.suiteName === 'Android Automation'; });
...
function runIOSSuite() {
  ...
  // Update start time
  const existing = trackingLog.find(function(e: any) { return e.suiteName === name; });
  if (existing) {
  ...
  var iosEntry = trackingLog.find(function(e: any) { return e.suiteName === name; });
  if (iosEntry) {
...
function durationMs(startISO: any) {
...
function saveExecutionStatus() {
  try {
    ...
    const enriched = trackingLog.map(function(entry: any) {
      var enriched = { ...entry };
...
function trackEntry(name: any, command: any, status: any, errorMessage: any, reportPath: any) {
  const existing = trackingLog.find(function(e: any) { return e.suiteName === name; });
  if (existing) {
...
function addTrackingEntry(name: any, command: any) {
  trackingLog.push({
    suiteName: name,
    command: command,
    status: 'NOT_EXECUTED',
...
function checkSuiteEnabled(suiteDef: any) {
...
function printSeparator(title: any) {
...
function main() {
  ...
  var executed = trackingLog.filter(function(r: any) { return r.status !== 'SKIPPED' && r.status !== 'NOT_EXECUTED'; }).length;
  var passed = trackingLog.filter(function(r: any) { return r.status === 'PASSED'; }).length;
  var failed = trackingLog.filter(function(r: any) { return r.status === 'FAILED'; }).length;
  var skipped = trackingLog.filter(function(r: any) { return r.status === 'SKIPPED'; }).length;
  ...
  trackingLog.forEach(function(r: any) {
    var icon;
