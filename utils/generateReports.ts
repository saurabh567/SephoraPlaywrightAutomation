...
function fileExists(relativePath: any) {
  return fs.existsSync(path.resolve(ROOT, relativePath));
}

function writeMetadata(platform: any) {
  ...
  const lines = (Object.entries(cfg.metadata) as [string, any][]).map(([k, v]) => `${k}=${v}`);
...
function generateCucumberHtml(platform: any) {
  const cfg = PLATFORMS[platform];
    ...
    try {
      ...
      fallbackReporter.generate({
        ...
        customData: {
          ...
          data: (Object.entries(cfg.metadata) as [string, any][]).map(([k, v]) => ({ label: k, value: v }))
        }
      });
...
function generateCombinedCucumberHtml() {
  ...
  try {
    // Stage ONLY the platform cucumber reports into a clean dir — the reporter
    // scans jsonDir recursively for ALL *.json and JSON.parse(...).map crashes
    // on non-array JSON (dashboard/ai/api summary files live under reports/).
    const stagingDir = path.resolve(ROOT, 'reports/combined/source');
    ensureDir(stagingDir);
    const staged = [];
    for (const p of ['web', 'android', 'ios']) {
      const jsonPath = path.resolve(ROOT, PLATFORMS[p].jsonReport);
      if (fs.existsSync(jsonPath)) {
        const dest = path.join(stagingDir, `${p}-cucumber-report.json`);
