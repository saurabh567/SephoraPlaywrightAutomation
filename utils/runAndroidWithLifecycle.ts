 ...
 */
function discoverLauncherActivity(pkg: any) {
  if (!pkg) return null;
...
function setFailedStage(stage: any) {
...
function waitFor(description: any, fn: any, timeoutMs = 120_000, intervalMs = 2000) {
  return new Promise((resolve, reject) => {
    ...
    const id = setInterval(() => {
      ...
      } catch (_: any) { /* ignore polling errors */ }
    }, intervalMs);
  });
}

function waitForWithPidCheck(description: any, fn: any, pid: any, timeoutMs = 120_000, intervalMs = 2000) {
  return new Promise((resolve, reject) => {
    const start = Date.now();
    const id = setInterval(() => {
      const elapsed = Date.now() - start;
...
function runCmd(cmd: any, opts: any = {}) {
  return execSync(cmd, {
    encoding: 'utf8',
    stdio: 'pipe',
    timeout: 30000,
...
function runCmdOptional(cmd: any, opts: any = {}) {
  try {
...
function isPortListeningSync(host: any, port: any) {
...
async function waitForPortFree(host: any, port: any, timeoutMs = 15000) {
  const start = Date.now();
...
async function waitForAppiumHealthy(host: any, port: any, timeoutMs = 60000) {
...
async function phaseStartEmulator() {
    ...
    if (!HEADLESS_EMULATOR) {
      ...
      try {
        ...
        if (isHeadless) {
          ...
          try { runCmd("adb emu kill 2>/dev/null || true", { timeout: 3000 }); } catch (_: any) {}
          await sleep(3000);
          try { runCmd("pkill -f .qemu-system. 2>/dev/null || true", { timeout: 5000 }); } catch (_: any) {}
          try { runCmd("pkill -f .emulator.*avd. 2>/dev/null || true", { timeout: 5000 }); } catch (_: any) {}
          await sleep(3000);
        } else {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const emu = spawn('emulator', [
      ...
      '-no-snapshot',
      '-no-audio',
      ...(HEADLESS_EMULATOR ? ['-no-window'] : [] as any[]),
    ...
    try {
      ...
      if (elapsed < 60 && attempt < maxAttempts) {
        ...
        try { process.kill(currentPid as number, 'SIGKILL'); } catch (_: any) {}
        await sleep(3000);
        continue;
      }
      throw err;
...
function parseCucumberJson(filePath: any) {
  try {
...
async function stopEmulator() {
  ...
  try {
    ...
    if (pidToKill) {
      try { process.kill(pidToKill, 'SIGTERM'); } catch (_: any) {}
      await sleep(3000);
      try { process.kill(pidToKill, 'SIGKILL'); } catch (_: any) {}
      await sleep(1000);
    }
  } catch (e: any) {
...
`;
  ...
  try { fs.mkdirSync(reportDir, { recursive: true }); } catch (_: any) {}
  ...
  if (!fs.existsSync(path.join(ROOT, 'reports', 'android', 'cucumber-report.json'))) {
    try {
      ...
      var lines: any[] = [];
      lines.push('# Android Automation Summary');
      lines.push('');
      lines.push('> Generated from execution lifecycle data');
      lines.push('> Time: ' + executionState.endTime);
      lines.push('');
      ...
      lines.push('| Raw Test Exit Code | ' + (executionState.rawTestExitCode !== null ? executionState.rawTestExitCode : 'N/A') + ' |');
      ...
      lines.push('## Notes');
      lines.push('');
      lines.push('Detailed Cucumber report was not generated. Cucumber exited with code ' + (executionState.rawTestExitCode !== null ? executionState.rawTestExitCode : 'N/A') + '.');
      lines.push('See the lifecycle summary for more details: reports/ai/android-execution-lifecycle-summary.md');
      lines.push('');
      lines.push('---');
      lines.push('*Generated by runAndroidWithLifecycle.js*');
      var summaryContent = lines.join('\n');
      fs.writeFileSync(androidSummaryPath, summaryContent, 'utf8');
      log('android summary written to ' + androidSummaryPath);
    } catch (e: any) {
      log('failed to write android summary: ' + e.message);
    }
  }
}
