...
class MobileDebugUtility {
   ...
   */
  async savePageSource(label: any) {
   ...
   */
  async saveScreenshot(label: any) {
    const timestamp = Date.now();
  ...
  async getInteractiveElements(visibleOnly = true) {
    try {
      ...
      if (visibleOnly) {
        return (elements || []).filter(function(e: any) { return e.visible; });
      }
      return elements || [];
    } catch (err: any) {
  ...
  async logPlatformContext() {
    let context = 'unknown';
    try { context = await this.driver.getContext(); } catch (_: any) {}
   ...
   */
  async captureDOMStructure(failedSelector: any, options: any = {}) {
    const depth = options.depth || 2;

    try {
      const domContext = await this.driver.execute(function(selector: any, maxDepth: any) {
        var result: any = {
          selector: selector,
          matchedElements: [] as any[],
   ...
   */
  async captureFailure(actionName: any, options: any = {}) {
    ...
    // 6. All contexts
    let contexts: any[] = [];
    try { contexts = await this.driver.getContexts(); } catch (_: any) {}
   ...
   */
  async logStep(actionLabel: any, opts: any = {}) {
    this._stepCounter++;
   ...
   */
  logStepEnd(actionLabel: any, startTime: any, success: any) {
   ...
   * @returns {Promise<object>} Diagnostics metadata
   */
  static async captureFailure(driver: any, scenarioName: any, error: any, failedSelectors: any) {
    if (!driver) {
   ...
   * @returns {Promise<object>} Dump metadata
   */
  static async dumpPageState(driver: any, label: any) {
    if (!driver) {
   ...
   */
  static async logInteractiveElements(driver: any, visibleOnly: any) {
    if (!driver) {
   ...
   */
  static async logEnvironment(driver: any) {
    if (!driver) {
      logger.warn('[MobileDebug] No driver available');
      return null;
    }
   ...
   */
  static async saveScreenshot(driver: any, label: any) {
   ...
   */
  static async savePageSource(driver: any, label: any) {
    if (!driver) return null;
    var debug = new MobileDebugUtility(driver);
...
export default MobileDebugUtility;
