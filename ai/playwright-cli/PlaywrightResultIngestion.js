...
class PlaywrightResultIngestion {
  ...
  _discoverArtifacts(artifacts) {
    // Trace files (.zip in test-results)
    if (fs.existsSync(TEST_RESULTS_DIR)) {
      const traceFiles = this._findFiles(TEST_RESULTS_DIR, name =>
    ...
    if (fs.existsSync(VIDEOS_DIR)) {
      artifacts.videoFiles = this._findFiles(VIDEOS_DIR, name => name.endsWith('.webm'));
    }

    // Screenshots (.png)
    if (fs.existsSync(SCREENSHOTS_DIR)) {
      artifacts.screenshotFiles = this._findFiles(SCREENSHOTS_DIR, name =>
    ...
    if (fs.existsSync(REPORTS_CLI_DIR)) {
      artifacts.jsonReports = this._findFiles(REPORTS_CLI_DIR, name => name.endsWith('.json'));
    }
    // Also check reports/ root for JSON
    if (fs.existsSync(REPORTS_DIR)) {
      const reportJsons = this._findFiles(REPORTS_DIR, name =>
    ...
    if (fs.existsSync(PW_REPORT_DIR)) {
      ...
      const allHtml = this._findFiles(PW_REPORT_DIR, name => name.endsWith('.html'));
    ...
    // JUnit XML reports
    if (fs.existsSync(REPORTS_CLI_DIR)) {
      artifacts.junitReports = this._findFiles(REPORTS_CLI_DIR, name => name.endsWith('.xml'));
    }
    if (fs.existsSync(REPORTS_DIR)) {
      const junits = this._findFiles(REPORTS_DIR, name => name.endsWith('.xml'));
    ...
    if (fs.existsSync(REPORTS_DIR)) {
      artifacts.cucumberReports = this._findFiles(REPORTS_DIR, name => name === 'cucumber-report.json');
    }
    ...
    for (const platform of ['web', 'android', 'ios', 'api']) {
      ...
      if (fs.existsSync(platformDir)) {
        const cukes = this._findFiles(platformDir, name => name === 'cucumber-report.json');
          ...
          for (const feature of data) {
            ...
            for (const el of elements) {
              if (el.type === 'scenario') {
                ...
                const hasSkip = steps.some(s => s.result && s.result.status === 'skipped');
                if (hasFail) failed++;
   ...
   */
  _canonicalizeReports(artifacts) {
   ...
   */
  _publishResults(artifacts, executionId) {
    try {
   ...
   */
  _writeIngestionManifest(artifacts, executionId) {
    try {
      const manifest = {
        executionId,
...
module.exports.PlaywrightResultIngestion = PlaywrightResultIngestion;
module.exports.ArtifactCollection = ArtifactCollection;
