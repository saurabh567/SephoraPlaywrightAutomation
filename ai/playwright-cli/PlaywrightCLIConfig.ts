...
const MODE_PROFILES: Record<string, any> = {
...
class PlaywrightCLIConfig {
   ...
   */
  resolveProfile(profileName: any, customOverrides: any = {}): any {
    ...
    for (const [key, value] of (Object.entries(customOverrides) as [string, any][])) {
      if (key === 'envOverrides') {
        resolved.envOverrides = { ...resolved.envOverrides, ...value };
      } else if (key === 'reporter') {
   ...
   */
  selectProfileFromContext(context: any = {}) {
   ...
   * @returns {Object|null}
   */
  getProfileMetadata(profileName: any) {
...
export { PlaywrightCLIConfig, MODE_PROFILES, BASE_PROFILE };
