...
class AgentExecutionSnapshot {
  [key: string]: any;
  constructor(agentKey: any, metadata: any = {}) {
    this.agentId = agentKey;
    this.agentName = metadata.name || agentKey;
    this.agentCategory = (CATEGORY_MAP as Record<string, string>)[agentKey] || metadata.executionStage || 'General';
    this.module = (MODULE_MAP as Record<string, string>)[agentKey] || metadata.module || 'General';
  ...
  _inferTriggerCondition(agentKey: any, metadata: any) {
    if (metadata.conditions && metadata.conditions.length > 0) {
      return metadata.conditions.map((c: any) => c.type || JSON.stringify(c)).join(', ');
    }
  ...
  setStatus(newStatus: any, reason = '') {
    const previousStatus = this.status;
    this.status = newStatus;
    this.statusIcon = (STATUS_ICONS as Record<string, string>)[newStatus] || '❓';
    this.reason = reason;
...
class AgentMonitor {
  ...
  start() {
    ...
    const subs = [
      this._eventBus.on('AgentRegistered', (event: any) => {
        this._handleAgentEvent('REGISTERED', event);
      }),
      this._eventBus.on('AgentInitialized', (event: any) => {
        this._handleAgentEvent('INITIALIZED', event);
      }),
      this._eventBus.on('AgentWaiting', (event: any) => {
        this._handleAgentEvent('WAITING', event);
      }),
      this._eventBus.on('AgentEligible', (event: any) => {
        this._handleAgentEvent('ELIGIBLE', event);
      }),
      this._eventBus.on('AgentStarted', (event: any) => {
        this._handleAgentStarted(event);
      }),
      this._eventBus.on('AgentCompleted', (event: any) => {
        this._handleAgentCompleted(event);
      }),
      this._eventBus.on('AgentFailed', (event: any) => {
        this._handleAgentFailed(event);
      }),
      this._eventBus.on('AgentSkipped', (event: any) => {
        this._handleAgentSkipped(event);
      }),
      this._eventBus.on('AgentRecovered', (event: any) => {
        this._handleAgentRecovered(event);
      }),
      this._eventBus.on('AgentDisabled', (event: any) => {
        this._handleAgentDisabled(event);
      }),

      // Also subscribe to execution events
      this._eventBus.on('ExecutionStarted', (event: any) => {
        this._startedAt = new Date().toISOString();
      }),
      this._eventBus.on('ExecutionCompleted', (event: any) => {
   ...
   */
  getSummary() {
    const snapshots: any[] = Array.from(this._snapshots.values());
  ...
  getByStatus(status: any) {
    return Array.from(this._snapshots.values()).filter((s: any) => s.status === status);
  }
  ...
  _handleAgentEvent(state: any, event: any) {
  ...
  _handleAgentCompleted(event: any) {
    const payload = event && event.payload ? event.payload : event;
    const agentKey = payload && (payload.key || payload.agentKey || payload.agentId || payload.name);
    if (!agentKey) return;
  ...
  _handleAgentRecovered(event: any) {
    const payload = event && event.payload ? event.payload : event;
    const agentKey = payload && (payload.key || payload.agentKey || payload.agentId || payload.name);
    if (!agentKey) return;
  ...
  _handleAgentDisabled(event: any) {
    const payload = event && event.payload ? event.payload : event;
    const agentKey = payload && (payload.key || payload.agentKey || payload.agentId || payload.name);
    if (!agentKey) return;
...
export { AgentMonitor, AgentExecutionSnapshot, STATUS_ICONS };
