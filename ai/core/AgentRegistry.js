...
const EXCLUDED_FILES = new Set(['index.js', 'baseAgent.js']);
...
class AgentRegistry {
  ...
  _inferMetadata(agentModule, filePath) {
    const baseName = path.basename(filePath, '.js');
    return {
      key: baseName, name: this._toName(baseName),
      version: '1.0.0', description: 'AI agent: ' + this._toName(baseName),
      dependencies: [], platforms: ['WEB', 'ANDROID', 'IOS', 'API'],
      tags: ['ai'], executionStage: 'analysis', priority: 50,
      conditions: [], retryPolicy: { maxRetries: 0, backoff: 'none' },
      lifecycle: 'active', _inferred: true
    };
  }

  _toName(camelCase) {
    return camelCase.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase()).replace(/Agent$/, '').trim() + ' Agent';
  }

  _resolveDependencyNames(entry) {
    const deps = entry.metadata.dependencies || [];
    entry._dependencyKeys = deps.map(dep => {
  ...
  getAll() { return Array.from(this._agents.values()); }
  get(key) { return this._agents.get(key) || null; }
  ...
  getByPlatform(platform) {
    ...
    return this.getAll().filter(a =>
      a.metadata.platforms && a.metadata.platforms.some(p => p.toUpperCase() === upper)
    );
  }
  ...
  getByTag(tag) {
    return this.getAll().filter(a =>
      a.metadata.tags && a.metadata.tags.some(t => t.toLowerCase() === tag.toLowerCase())
    );
  }
  ...
  getStats() {
    ...
    const byStage = {}; const byLifecycle = {}; const byPlatform = {};
    for (const a of all) {
