...
class AgentRunner {
  ...
  writePostTestSummary(results: any) {
    ...
    const content = [
      ...
      '| Agent | Output | Status |',
      '|---|---|---|',
      ...results.map((result: any) => `| ${result.agent} | ${result.outputPath || ''} | ${result.error ? 'failed' : 'completed'} |`)
    ].join('\n');
  ...
  writeMemory(runType: any, results: any) {
