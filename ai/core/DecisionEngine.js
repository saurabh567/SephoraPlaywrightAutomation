...
class DecisionEngine {
  ...
  _getFailureHistory() {
    try {
      ...
      return store.getTopFailures(10).map(f => ({
        scenario: f.scenario,
        feature: f.feature,
        count: f.count,
        lastSeen: f.lastSeen
  ...
  _getGitChanges() {
    try {
      ...
      return output.trim().split('\n').filter(Boolean).map(l => l.replace(/^[MARC??]+\s+/, '').trim()).filter(Boolean);
    } catch (e) {
      return [];
    }
  }
  ...
  _assessRisk(context) {
    ...
    const recentFailures = context.failureHistory.filter(f => {
    ...
    // Git changes risk
    const criticalFiles = context.gitChanges.filter(f =>
   ...
   */
  _decideAgents(context) {
    const agents = { always: [], conditional: [], skip: [] };
   ...
   */
  _decideHealingStrategy(context) {
    const healing = {};
