...
class EventBus {
   ...
   */
  on(eventType: any, callback: any, options: any = {}) {
   ...
   */
  once(eventType: any, callback: any) {
   ...
   */
  onAny(eventTypes: any, callback: any, options: any = {}) {
    const unsubs = eventTypes.map((type: any) => this.on(type, callback, options));
   ...
   */
  onFiltered(eventType: any, callback: any, filterFn: any) {
   ...
   */
  emit(eventType: any, payload: any = {}, options: any = {}) {
    if (!this._enabled) return;
   ...
   */
  registerCustomEvent(eventName: any, description: any) {
   ...
   */
  loadCustomEventsFromDir(customEventsDir: any) {
    const results: any = [];
  ...
  getHistory(options: any = {}) {
    ...
    if (options.eventType) {
      events = events.filter((e: any) => e.type === options.eventType);
    }
  ...
  getActiveTopics() {
    ...
    return events.filter((e: any) => e !== '*' && this._emitter.listenerCount(e) > 0);
  }
   ...
   */
  _normalizeEvent(eventType: any, rawPayload: any, options: any = {}) {
...
const legacyComm = {
  publish: (topic: any, payload: any) => instance.emit(topic, payload),
  subscribe: (topic: any, cb: any) => instance.on(topic, cb),
  log: (agentName: any, message: any) => console.log(`[AgentComm][${agentName}] ${message}`)
};

// Re-export legacy AgentComm for backward compatibility
export default instance;
export { EventBus, EVENTS };
export const AgentComm = legacyComm;;
export const legacy = legacyComm;;
