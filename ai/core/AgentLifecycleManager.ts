...
class AgentLifecycleEntry {
   ...
   */
  transitionTo(newState: any, timestamp: any) {
...
class AgentLifecycleManager {
  ...
  register(agentEntry: any) {
    ...
    if (entry.lifecycle === 'deprecated' || entry.lifecycle === 'placeholder') {
      entry.transitionTo(STATES.DISABLED, 'lifecycle deprecated/placeholder');
   ...
   */
  running(agentKey: any, phaseId: any) {
   ...
   */
  completed(agentKey: any, result: any = {}) {
   ...
   */
  recovered(agentKey: any, result: any = {}) {
   ...
   */
  skipped(agentKey: any, reason: any) {
    if (!reason) {
   ...
   */
  disable(agentKey: any, reason: any) {
  ...
  load() {
    ...
    try {
      ...
      if (data.agents) {
        for (const [key, entryData] of (Object.entries(data.agents) as [string, any][])) {
   ...
   */
  _transition(agentKey: any, newState: any) {
    const entry = this._agents.get(agentKey);
    if (!entry) {
   ...
   */
  _emitEvent(entry: any, state: any) {
    try {
      ...
      const eventType = (eventMap as Record<string, any>)[state] || 'AgentLifecycle' + state;
      const payload = {
        agent: entry.key,
        name: entry.name,
        state: state,
...
export { AgentLifecycleManager, STATES, AgentLifecycleEntry, SKIP_REASONS };
