...
import { resolveNpx, buildNpxPlaywrightArgs } from '../../utils/resolveBinaryPath';
...
class PlaywrightExecutionEngine {
  ...
  async execute(options: any = {}) {
    ...
    try {
      const spawnResult: any = await this._spawn(NPX_PW, cliArgs, env);
   ...
   */
  buildCommand(config: any, options: any = {}) {
    const args: any[] = [];
   ...
   */
  _spawn(command: any, args: any, env: any) {
   ...
   */
  _resolveConfig(profileName: any, platform: any, options: any) {
   ...
   */
  _selectProfile(platform: any, options: any) {
   ...
   */
  _validateEnvironment(config: any) {
    const errors: any[] = [];
   ...
   */
  _locateArtifacts(result: any, config: any) {
    // HTML report
    if (config.reporter) {
    ...
    if (fs.existsSync(TEST_RESULTS_DIR)) {
      const traceFiles = this._findFiles(TEST_RESULTS_DIR, (name: any) => name.endsWith('.zip') && name.includes('trace'));
      if (traceFiles.length > 0) result.tracePath = traceFiles[0];
    }

    // Video files
    if (fs.existsSync(TEST_RESULTS_DIR)) {
      const videoFiles = this._findFiles(TEST_RESULTS_DIR, (name: any) => name.endsWith('.webm'));
   ...
   * Print execution banner.
   */
  _printBanner(platform: any, profileName: any, config: any) {
    console.log(`\n╔══════════════════════════════════════════════╗`);
...
export { PlaywrightExecutionEngine, CanonicalExecutionResult };
