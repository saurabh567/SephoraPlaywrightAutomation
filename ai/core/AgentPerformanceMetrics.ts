...
class AgentPerformanceEntry {
  ...
  _recomputeSummary() {
    ...
    this.summary.totalDuration = ex.reduce((s: any, e: any) => s + e.duration, 0);
    this.summary.meanDuration = Math.round(this.summary.totalDuration / len);
    this.summary.minDuration = Math.min(...ex.map((e: any) => e.duration));
    this.summary.maxDuration = Math.max(...ex.map((e: any) => e.duration));

    this.summary.totalMemory = ex.reduce((s: any, e: any) => s + (e.memory || 0), 0);
    this.summary.meanMemory = len > 0 ? Math.round(this.summary.totalMemory / len) : 0;
    this.summary.peakMemory = Math.max(...ex.map((e: any) => e.peakMemory || e.memory || 0));

    this.summary.totalCPU = ex.reduce((s: any, e: any) => s + (e.cpu || 0), 0);
    this.summary.meanCPU = len > 0 ? Math.round((this.summary.totalCPU / len) * 100) / 100 : 0;

    this.summary.failures = ex.filter((e: any) => e.status === 'failed').length;
    this.summary.successes = ex.filter((e: any) => e.status === 'completed').length;
    this.summary.skips = ex.filter((e: any) => e.status === 'skipped').length;
    this.summary.recoveries = ex.filter((e: any) => e.status === 'recovered').length;
    this.summary.retries = ex.reduce((s: any, e: any) => s + (e.retries || 0), 0);
  ...
  _calculateTrend(ex: any) {
    ...
    const recentAvg = recent.reduce((s: any, e: any) => s + e.duration, 0) / recent.length;
    const olderAvg = older.reduce((s: any, e: any) => s + e.duration, 0) / older.length;

    // Also check failure rate trend
    const recentFailures = recent.filter((e: any) => e.status === 'failed').length;
    const olderFailures = older.filter((e: any) => e.status === 'failed').length;
...
class AgentPerformanceMetrics {
   ...
   */
  async record(agentKey: any, data: any = {}) {
   ...
   */
  getSummary() {
    const all = Array.from(this._cache.values()) as any[];
    const byTrend: Record<string, any> = {};
    let totalDuration = 0;
    let totalMemory = 0;
    let totalFailures = 0;
   ...
   * @returns {Array}
   */
  getTrend(agentKey: any, limit = 20) {
    const entry = this._cache.get(agentKey);
    if (!entry) return [];
    return entry.executions.slice(-limit).map((e: any) => ({
   ...
   * @returns {Array}
   */
  getTopByMetric(metric: any, limit = 10) {
    const all = Array.from(this._cache.values()) as any[];
   ...
   */
  getDegradingAgents() {
    return (Array.from(this._cache.values()) as any[])
      .filter((e: any) => e.summary.trend === 'degrading')
      .map((e: any) => ({
        key: e.agentKey,
        meanDuration: e.summary.meanDuration,
        successRate: e.summary.successRate,
   ...
   */
  async syncFromLifecycle(lifecycleManager: any) {
...
export { AgentPerformanceMetrics, AgentPerformanceEntry };
