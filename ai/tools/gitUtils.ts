...
function runGit(args: any, opts: any = {}) {
...
function isPathForbiddenByPattern(relativePath: any) {
 ...
 */
function isAiPathAllowed(relativePath: any) {
  const normalized = relativePath.replace(/\\/g, '/');
 ...
 */
function validateFilesToStage(files: any) {
  const accepted: any[] = [];
  const rejected: any[] = [];
...
export default { getStatus() {
    ...
    return { ok: r.ok, staged: toAdd, rejected: [] as any[], gitResult: r };
  }, createCommit(message: any, options: any = {}) {
