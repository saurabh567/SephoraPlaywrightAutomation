...
function hashId(value: any) {
  return crypto.createHash('sha256').update(value).digest('hex');
}

function listFiles(directory: any, extensions: any[] = []): any[] {
  if (!fs.existsSync(directory)) return [];
  return fs.readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    const fullPath = path.join(directory, entry.name);
    if (entry.isDirectory()) return listFiles(fullPath, extensions);
    return extensions.length === 0 || extensions.includes(path.extname(entry.name).toLowerCase()) ? [fullPath] : [] as any[];
  });
}

function chunkText(content: any, chunkSize = config.chunkSize, overlap = config.chunkOverlap) {
...
function scalarMetadata(metadata: any) {
  return Object.fromEntries(
    (Object.entries(metadata) as [string, any][]).filter(([, value]) =>
      typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean'
    )
  );
}

function redactSensitiveData(content: any) {
...
class IngestionService {
  ...
  buildFileRecords(collectionName: any, filePath: any, type: any, extraMetadata: any = {}) {
  ...
  buildLocatorRecords(filePath: any) {
    const relativePath = path.relative(config.rootDir, filePath);
  ...
  buildCucumberFailureRecords(reportPath: any) {
    ...
    return report.flatMap((feature) =>
      (feature.elements || []).flatMap((scenario: any) => {
  ...
  buildCucumberReportSummary(reportPath: any) {
    let report;
    try {
      report = fs.readJsonSync(reportPath);
    } catch (error: any) {
    ...
    const failed = scenarios.filter((scenario) =>
      (scenario.steps || []).some((step: any) => step.result?.status === 'failed')
    ).length;
    const skipped = scenarios.filter((scenario) => {
      const statuses = (scenario.steps || []).map((step: any) => step.result?.status);
      return statuses.length > 0 && statuses.every((status: any) => status === 'skipped');
    }).length;
    const relativePath = path.relative(config.rootDir, reportPath);
    const document = [
  ...
  collectFrameworkDocuments(options: any = {}) {
    ...
    if (includeRuntime) {
      ...
      const jenkinsFiles = [
        ...
        ...listFiles(path.join(root, 'reports'), ['.log']).filter((file: any) => /jenkins/i.test(file))
      ];
  ...
  async ingestDocuments(documents: any) {
    await this.client.healthCheck();
    this.embeddingService.validateConfiguration();
    await this.client.ensureCollections((Object.values(config.collections) as any[]));

    const grouped = documents.reduce((result: any, document: any) => {
    ...
    const results: any[] = [];
    for (const [collectionName, collectionDocuments] of (Object.entries(grouped) as [string, any][])) {
      const embeddings = await this.embeddingService.embedMany(collectionDocuments.map((item: any) => item.document));
      const records = collectionDocuments.map((document: any, index: any) => ({
        ...document,
