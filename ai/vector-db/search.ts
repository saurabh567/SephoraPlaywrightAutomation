...
function parseArgs(argv: any) {
  const args: Record<string, any> = { type: 'failures', query: '', top: 5 };
...
function resolveCollection(type: any) {
  const map = {
  ...
  return (map as Record<string, any>)[type] || config.collections.failures;
}
