...
function classifyIssues(summary: any, thresholds: any) {
  const issues: any[] = [];
  ...
  if (summary.byLabel && summary.byLabel.length >= 2) {
    const responseTimes = summary.byLabel.map((l: any) => l.avg);
...
async function generateReport(summary: any, thresholds: any, issues: any, jtlPath: any) {
  const now = new Date().toISOString();
  const overallSeverity = issues.some((i: any) => i.severity === 'critical') ? 'CRITICAL'
    : issues.some((i: any) => i.severity === 'high') ? 'HIGH'
    : issues.some((i: any) => i.severity === 'medium') ? 'MEDIUM'
    : 'LOW';
