...
function loadState() {
  ...
  if (!fs.existsSync(READINESS_STATE_PATH)) {
    fs.writeJsonSync(READINESS_STATE_PATH, { checks: [] as any[] }, { spaces: 2 });
  }
...
var EcosystemReadinessAgent = {
  ...
  checkHealth: function() {
    ...
    // Compute summary
    var allChecks: any = [];
    Object.keys(health).forEach(function(category: any) {
      if (category === 'checkedAt') return;
      if (Array.isArray(health[category])) {
        health[category].forEach(function(check: any) {
          if (check.status) allChecks.push(check);
        });
      }
    });

    var passed = allChecks.filter(function(c: any) { return c.status === 'OK'; }).length;
    var warned = allChecks.filter(function(c: any) { return c.status === 'WARN' || c.status === 'NOT YET RUN' || c.status === 'NO ALERTS'; }).length;
    var failed = allChecks.filter(function(c: any) { return c.status === 'FAIL' || c.status === 'MISSING' || c.status === 'NOT CONFIGURED'; }).length;
  ...
  run: function(input: any) {
    ...
    lines.push('|---|---|---|');
    var agentFailures = 0;
    health.agentRegistry.forEach(function(a: any) {
    ...
    lines.push('|---|---|');
    health.framework.forEach(function(c: any) {
    ...
    lines.push('|---|---|---|');
    health.testAssets.forEach(function(c: any) {
    ...
    lines.push('| Component | Status |');
    lines.push('|---|---|');
    health.infrastructure.forEach(function(c: any) {
    ...
    lines.push('| Component | Status |');
    lines.push('|---|---|');
    health.mobileSupport.forEach(function(c: any) {
    ...
    lines.push('| Component | Status |');
    lines.push('|---|---|');
    health.reporting.forEach(function(c: any) {
    ...
    phaseMap.forEach(function(p) {
      var phaseAgent = health.agentRegistry.find(function(a: any) { return a.name && (p[2] as string).indexOf(a.name) !== -1; });
    ...
    if (failedItems.length > 0) {
      failedItems.forEach(function(item: any) {
        lines.push('- &#x26A0;&#xFE0F; ' + item);
      });
    } else {
    ...
    return {
      ...
      allAgentsLoaded: health.agentRegistry.filter(function(a: any) { return a.status === 'OK'; }).length === health.agentRegistry.length,
...
export default EcosystemReadinessAgent;
