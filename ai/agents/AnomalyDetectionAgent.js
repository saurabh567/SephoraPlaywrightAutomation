...
function loadState() {
  ...
  if (!fs.existsSync(ANOMALY_STATE_PATH)) {
    fs.writeJsonSync(ANOMALY_STATE_PATH, { history: [], alerts: [] }, { spaces: 2 });
  }
...
function extractScenarioResults() {
  ...
  features.forEach(function(f) {
    (f.elements || []).forEach(function(el) {
      if (el.type !== 'scenario') return;
      var failed = (el.steps || []).some(function(st) { return st.result && st.result.status === 'failed'; });
      var skipped = (el.steps || []).every(function(st) { return st.result && (st.result.status === 'skipped' || st.result.status === 'undefined'); });
      var passed = !failed && !skipped;
      var duration = (el.steps || []).reduce(function(sum, st) {
...
function detectFlakyTests(state) {
  var flaky = [];
  var scenarioMap = {};
  ...
  Object.keys(scenarioMap).forEach(function(scenario) {
    ...
    if (results.length >= 3) {
      var passedCount = results.filter(function(r) { return r; }).length;
  ...
  return flaky.sort(function(a, b) { return a.passRate - b.passRate; });
}

// Detect performance regressions
function detectPerformanceRegression(state) {
  var regressions = [];
  var scenarioDurationMap = {};
  ...
  return regressions.sort(function(a, b) { return b.changePercent - a.changePercent; });
}

// Detect locator decay patterns from failure messages
function detectLocatorDecay(scenarios) {
  var locatorErrors = [];
...
function generateAlerts(flaky, regressions, locatorIssues, state) {
  var alerts = [];
  var now = timestamp();

  // Flaky test alerts
  flaky.forEach(function(f) {
    var existing = (state.alerts || []).find(function(a) {
  ...
  locatorIssues.forEach(function(l) {
    var existing = (state.alerts || []).find(function(a) {
...
var AnomalyDetectionAgent = {
  ...
  run: function(input) {
    ...
    if (scenarios.length === 0) {
      ...
      return { ok: true, message: 'No scenario data available for analysis', alerts: [], anomalies: {} };
    }
    ...
    var currentRun = {
      ...
      passed: scenarios.filter(function(s) { return s.passed; }).length,
      failed: scenarios.filter(function(s) { return s.failed; }).length,
    };
    ...
    var anomalies = {
      ...
      openAlerts: (state.alerts || []).filter(function(a) { return a.status === 'open'; }).length,
    };
    ...
    lines.push('');
    var openAlerts = (state.alerts || []).filter(function(a) { return a.status === 'open'; });
  ...
  getActiveAlerts: function() {
    var state = loadState();
    return (state.alerts || []).filter(function(a) { return a.status === 'open'; });
  },
...
function main() {
  ...
  if (command === 'run' || command === 'analyze') {
    var result = AnomalyDetectionAgent.run();
  ...
  if (command === 'resolve') {
    ...
    var result = AnomalyDetectionAgent.resolveAlert(alertId);
    console.log(JSON.stringify(result, null, 2));
    return;
  }
