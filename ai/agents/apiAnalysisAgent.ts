...
// ---------------------------------------------------------------------------
function classifyIssues(results: any, summary: any) {
  const issues: any[] = [];
  ...
  const slow = results.filter((r: any) => r.responseTime > slowThreshold);
  if (slow.length > 0) {
    for (const s of slow) {
      issues.push({
  ...
  const unexpectedStatus = results.filter((r: any) =>
...
// ---------------------------------------------------------------------------
function computeHealthScore(results: any) {
  ...
  const slowCount = results.filter((r: any) => r.responseTime > 5000).length;
...
// ---------------------------------------------------------------------------
async function generateReport(summary: any, issues: any) {
  const now = new Date().toISOString();
  const results = summary.results || [];
  const total = summary.totalAPIs || results.length;
  const passed = summary.passed || results.filter((r: any) => r.passed).length;
  const failed = summary.failed || results.filter((r: any) => !r.passed).length;
  ...
  // Method distribution
  const methodCounts: Record<string, any> = {};
  results.forEach((r: any) => {
  ...
  lines.push('|--------|-------|');
  (Object.entries(methodCounts) as [string, any][]).sort().forEach(([method, count]) => {
    lines.push(`| ${method} | ${count} |`);
  });
  lines.push('');
  ...
  lines.push('|-------------|-------|----------------|');
  (Object.entries(statusCounts) as [string, any][]).sort(([a], [b]) => (Number(a) - Number(b))).forEach(([code, count]) => {
  ...
  if (allNormal) {
    ...
    const bySeverity = { critical: [] as any[], high: [] as any[], medium: [] as any[], info: [] as any[] };
    issues.forEach((i: any) => { (bySeverity as Record<string, any>)[i.severity] = (bySeverity as Record<string, any>)[i.severity] || []; (bySeverity as Record<string, any>)[i.severity].push(i); });
  ...
  const slow = results.filter((r: any) => r.responseTime > 5000);
  ...
  if (results.length > 0) {
    const times = results.map((r: any) => r.responseTime);
    const avg = Math.round(times.reduce((a: any, b: any) => a + b, 0) / times.length);
...
async function run(input: any = {}) {
  ...
  return {
    ...
    passed: summary.passed || results.filter((r: any) => r.passed).length,
    failed: summary.failed || results.filter((r: any) => !r.passed).length,
...
export default ApiAnalysisAgentWrapper;
