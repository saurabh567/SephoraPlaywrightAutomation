...
function generateNativePageObject(platform: any) {
...
function generateHybridPageObject(platform: any) {
  const className = platform === 'android'
...
function generateAndroidLocatorFile(name: any, locatorMap: any) {
  const lines: any[] = [];
  ...
  const entries = (Object.entries(locatorMap) as [string, any][]);
...
function generateIosLocatorFile(name: any, locatorMap: any) {
  const lines: any[] = [];
...
function generateAndroidFlow(flowName: any) {
  const lines: any[] = [];
  ...
  lines.push(`  }`);
  lines.push(``);
  lines.push(`  async execute(options: any = {}) {`);
...
function generateIosFlow(flowName: any) {
  const lines: any[] = [];
...
export default MobileTestGenerationAgent;
