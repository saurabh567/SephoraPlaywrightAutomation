...
class LocatorConfidenceScorer {
   ...
   */
  _scoreDomSimilarity(failed: any, candidate: any, domContext: any) {
    if (!failed || !candidate) return 0;
    if (failed === candidate) return 1.0;

    // Check for shared CSS class references
    const failedClasses = (failed.match(/\.([\w-]+)/g) || []).map((c: any) => c.slice(1));
    const candidateClasses = (candidate.match(/\.([\w-]+)/g) || []).map((c: any) => c.slice(1));
    const sharedClasses = failedClasses.filter((c: any) => candidateClasses.includes(c));
    ...
    const sharedTags = failedTags.filter((t: any) => candidateTags.includes(t));
    ...
    const failedAttrs = (failed.match(/\[([^\]]+)\]/g) || []).map((a: any) => a.slice(1, -1).split('=')[0]);
    const candidateAttrs = (candidate.match(/\[([^\]]+)\]/g) || []).map((a: any) => a.slice(1, -1).split('=')[0]);
    const sharedAttrs = failedAttrs.filter((a: any) => candidateAttrs.includes(a));
   ...
   */
  _scoreHistoricalSuccess(pageName: any, locatorName: any) {
   ...
   */
  _scoreLocatorTypeStability(locator: any, locatorType: any) {
    ...
    let typeScore = (stabilityMap as Record<string, number>)[locatorType as string] || 0.60;
   ...
   */
  _scoreAttributeRichness(locator: any) {
   ...
   */
  _scoreProximityMatch(failedLocator: any, candidateLocator: any) {
    ...
    const failedParts = failedLocator.split('>').map((s: any) => s.trim());
    const candidateParts = candidateLocator.split('>').map((s: any) => s.trim());
   ...
   */
  _diceSimilarity(a: any, b: any) {
    if (!a || !b) return 0;
    if (a === b) return 1.0;

    const bigrams: Record<string, any> = {};
   ...
   */
  isAutoApplyCandidate(score: any, threshold = 0.80) {
   ...
   */
  getRecommendedAction(score: any) {
    const label = this._classifyConfidence(score);
    const actions = {
...
export default LocatorConfidenceScorer;
