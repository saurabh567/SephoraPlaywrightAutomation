...
function loadState() {
  ...
  if (!fs.existsSync(retryStatePath)) {
    fs.writeJsonSync(retryStatePath, { retryHistory: [], currentRetry: null }, { spaces: 2 });
  }
...
module.exports = {
  run: async function run(input = {}) {
    ...
    // Find failed scenarios from the cucumber report
    ...
    // Attempt selective retry using tags
    let allPassed = false;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      // Create a temporary tag-based run for failed scenarios
