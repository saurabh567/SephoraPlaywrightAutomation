    ...
    const failures = input.failures || (() => {
      try {
        ...
        const scenarios = Array.isArray(report)
          ? report.flatMap(f => (f.elements || []).filter((e: any) => e.type === 'scenario'))
          : [] as any[];
        return scenarios
          .filter(s => (s.steps || []).some((st: any) => st.result?.status === 'failed'))
          .map(s => {
            const failedStep = (s.steps || []).find((st: any) => st.result?.status === 'failed');
            return {
