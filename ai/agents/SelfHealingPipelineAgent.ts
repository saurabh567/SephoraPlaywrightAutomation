...
function loadPipelineState() {
  ...
  if (!fs.existsSync(PIPELINE_STATE_PATH)) {
    fs.writeJsonSync(PIPELINE_STATE_PATH, { runs: [] as any[], currentRun: null }, { spaces: 2 });
  }
  return fs.readJsonSync(PIPELINE_STATE_PATH);
}

function savePipelineState(state: any) {
...
function safeRequire(modulePath: any) {
  try {
...
const SelfHealingPipelineAgent = {
  ...
  _runPlatformTests(platform: any, envOverrides: any = {}) {
  ...
  async runSelfHealingCycle(platform: any, options: any = {}) {
  ...
  async _executeHealing(platform: any, attempt: any) {
    try {
      // 1. Failure analysis
      const failureAgent = safeRequire(path.join(AGENTS_DIR, 'failureAnalysisAgent'));
      let analysisResult: any = { skipped: true };
      ...
      let healResult: any = { skipped: true };
  ...
  async runCrossPlatform(platforms: any[] = [], options: any = {}) {
    ...
    // Compute summary
    const passed = pipelineRun.cycles.filter((c: any) => c.finalStatus === 'passed').length;
    const failed = pipelineRun.cycles.filter((c: any) => c.finalStatus === 'failed').length;
    const totalAttempts = pipelineRun.cycles.reduce((sum: any, c: any) => sum + c.attempts.length, 0);
    const totalHeals = pipelineRun.cycles.reduce((sum: any, c: any) => sum + c.healAttempts.length, 0);
    ...
    return {
      ...
      cycles: pipelineRun.cycles.map((c: any) => ({
        platform: c.platform,
...
export default SelfHealingPipelineAgent;
