...
function loadExecutionState() {
  ...
  if (!fs.existsSync(EXECUTION_STATE_PATH)) {
    fs.writeJsonSync(EXECUTION_STATE_PATH, { runs: [] as any[], activeDevices: [] as any[] }, { spaces: 2 });
  }
  return fs.readJsonSync(EXECUTION_STATE_PATH);
}

function saveExecutionState(state: any) {
  fs.writeJsonSync(EXECUTION_STATE_PATH, state, { spaces: 2 });
}

function getCapabilities(deviceConfig: any, appConfig: any = {}) {
...
const MobileDeviceFarmAgent = {
  ...
  listDevices() {
    return (Object.entries(BUILTIN_DEVICES) as [string, any][]).map(([key, dev]) => ({
      id: key,
  ...
  async runOnDevice(deviceConfig: any, appConfig: any = {}, testOptions: any = {}) {
    ...
    const runInfo: Record<string, any> = {
      ...
      artifacts: { reports: [] as any[], logs: [] as any[] },
    };
    ...
    try {
      ...
      const env: Record<string, any> = {
        ...
        APPIUM_HOST: deviceConfig.cloudProvider
          ? CLOUD_PROVIDERS[deviceConfig.cloudProvider as keyof typeof CLOUD_PROVIDERS]?.serverUrl?.split('://')[1]?.split(':')[0]
          : appiumConfig.host,
      ...
      state.activeDevices = state.activeDevices.filter((d: any) => d.runId !== runId);
  ...
  async runParallel(deviceConfigs: any, appConfig: any = {}, testOptions: any = {}) {
    ...
    const promises = deviceConfigs.map((deviceCfg: any) =>
  ...
  async runAndroidEmulator(appConfig: any = {}, testOptions: any = {}) {
  ...
  async runIOSSimulator(appConfig: any = {}, testOptions: any = {}) {
    return this.runOnDevice(BUILTIN_DEVICES['ios-simulator'], appConfig, testOptions);
  },

  // Run on a real Android device via USB
  async runAndroidRealDevice(udid: any, appConfig: any = {}, testOptions: any = {}) {
    const deviceCfg = {
  ...
  async runIOSRealDevice(udid: any, appConfig: any = {}, testOptions: any = {}) {
    const deviceCfg = {
      id: `ios-real-${udid}`,
      platform: 'ios',
      type: 'real',
  ...
  async runOnBrowserStack(deviceConfig: any = {}, appConfig: any = {}, testOptions: any = {}) {
  ...
  async runOnSauceLabs(deviceConfig: any = {}, appConfig: any = {}, testOptions: any = {}) {
  ...
  async run(input: any = {}) {
    ...
    if (mode === 'parallel' && devices.length > 0) {
      const deviceConfigs = devices.map((d: any) => {
        if (typeof d === 'string') {
          return BUILTIN_DEVICES[d as keyof typeof BUILTIN_DEVICES] || {
            id: d,
            platform,
            deviceName: d,
            type: 'emulator',
...
async function main() {
  ...
  if (command === 'parallel') {
    ...
    const deviceConfigs = platforms.map((p) =>
      BUILTIN_DEVICES[`${p}-emulator` as keyof typeof BUILTIN_DEVICES] ||
      BUILTIN_DEVICES[`${p}-simulator` as keyof typeof BUILTIN_DEVICES] || {
        id: p,
        platform: p,
...
export default MobileDeviceFarmAgent;
