...
function audit(action: any, detail: any) {
  ensureDirs();
...
function deriveKey(masterPassword: any) {
...
function loadVault() {
  ...
  if (!fs.existsSync(SECRETS_FILE)) {
    fs.writeJsonSync(SECRETS_FILE, { version: 1, profiles: {} as Record<string, any> }, { spaces: 2 });
  }
  try { return fs.readJsonSync(SECRETS_FILE); } catch { return { version: 1, profiles: {} as Record<string, any> }; }
}
...
function loadManifest() {
  ...
  if (!fs.existsSync(MANIFEST_FILE)) {
    var m = { createdAt: timestamp(), secretsCount: 0, profiles: [] as any[], lastRotated: null, keyHash: null };
    fs.writeJsonSync(MANIFEST_FILE, m, { spaces: 2 });
    return m;
  }
  try { return fs.readJsonSync(MANIFEST_FILE); } catch { return { createdAt: timestamp(), secretsCount: 0, profiles: [] as any[], lastRotated: null, keyHash: null }; }
}
...
var VAULT_PROFILES: Record<string, any> = {
  browserstack: {
...
var SecretsVaultAgent = {
  ...
  setSecret: function(profile: any, field: any, value: any) {
  ...
  getSecret: function(profile: any, field: any) {
    var key = getOrCreateVaultKey();
    var vault = loadVault();
  ...
  setProfile: function(profile: any, values: any) {
    var profileDef = VAULT_PROFILES[profile];
    if (!profileDef) return { ok: false, error: 'Unknown profile: ' + profile };

    var results: any = [];
    Object.keys(values).forEach(function(this: any, field: any) {
    ...
    var ok = results.every(function(r: any) { return r.ok; });
    return { ok: ok, profile: profile, results: results };
  },

  // Inject vault secrets into process.env (for runtime use)
  injectToEnv: function(profile: any) {
    var profileDef = VAULT_PROFILES[profile];
    if (!profileDef) return { ok: false, error: 'Unknown profile: ' + profile };

    var injected: any = [];
    Object.keys(profileDef.fields).forEach(function(this: any, field: any) {
    ...
    profileDef.envVars.forEach(function(this: any, envVar: any) {
  ...
  injectAllToEnv: function() {
    ...
    Object.keys(VAULT_PROFILES).forEach(function(this: any, profile: any) {
  ...
  generateEnvFile: function(outputPath: any, profiles: any) {
    ...
    profiles.forEach(function(this: any, profile: any) {
      var profileDef = VAULT_PROFILES[profile];
      ...
      profileDef.envVars.forEach(function(this: any, envVar: any) {
        ...
        var alreadyOutput = Object.keys(profileDef.fields).some(function(f: any) { return profileDef.fields[f] === envVar; });
        if (!alreadyOutput) {
  ...
  run: function(input: any) {
    ...
    var allSet = profiles.filter(function(p: any) { return p.allSecretsStored; }).length;
...
function main() {
  ...
  if (command === 'init') {
    let result = SecretsVaultAgent.init();
    console.log(JSON.stringify(result, null, 2));
    process.exit(result.ok ? 0 : 1);
  }

  if (command === 'status' || command === 'list') {
    let result = SecretsVaultAgent.getStatus();
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  ...
  if (command === 'set') {
    ...
    let result = SecretsVaultAgent.setSecret(profile, field, value);
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  ...
  if (command === 'set-profile') {
    ...
    let result = SecretsVaultAgent.setProfile(profile, values);
    console.log(JSON.stringify(result, null, 2));
    return;
  }

  if (command === 'inject') {
    let profile = args[1] || undefined;
    let result = profile ? SecretsVaultAgent.injectToEnv(profile) : SecretsVaultAgent.injectAllToEnv();
    console.log(JSON.stringify(result, null, 2));
    return;
  }

  if (command === 'generate-env') {
    let outputPath = args[1] || undefined;
    let result = SecretsVaultAgent.generateEnvFile(outputPath, Object.keys(VAULT_PROFILES));
    console.log(JSON.stringify(result, null, 2));
    return;
  }

  if (command === 'rotate') {
    let result = SecretsVaultAgent.rotateKey();
    console.log(JSON.stringify(result, null, 2));
    return;
  }
...
export default SecretsVaultAgent;
