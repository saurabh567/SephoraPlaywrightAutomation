...
function buildQuery(summary: any) {
  return (summary.failures || []).map(function(failure: any) {
    return [
      failure.feature,
      failure.scenario,
      failure.failedStep,
...
function classifyError(errorText: any) {
...
agent.analyzeLocally = function analyzeLocally(summary: any) {
  ...
  // Count by category
  var categoryCounts: Record<string, any> = {};
...
agent.analyzeWithRag = async function analyzeWithRag(input: any) {
  if (!input) input = {};
  ...
  var result = await rag.generate({
    ...
    retrieve: function(retrieval: any, topK: any) {
