...
function loadState() {
  ...
  if (!fs.existsSync(SELECTOR_STATE_PATH)) {
    fs.writeJsonSync(SELECTOR_STATE_PATH, { selections: [] as any[] }, { spaces: 2 });
  }
...
function mapChangedFilesToFeatures(changedFiles: any) {
  var affected = new Set();
  var affectedPages = new Set();
  var affectedTags = new Set();
  var risks: any = [];

  changedFiles.forEach(function(file: any) {
...
function determineTestStrategy(analysis: any, options: any) {
  var strategy = {
    runAll: false,
    runSmoke: false,
    runRegression: false,
    runCustomTags: [] as any[],
    features: [] as any[],
    priority: 'normal',
    estimatedTime: '~2 min',
    description: '',
  ...
  var hasCritical = analysis.risks.some(function(r: any) { return r.risk === 'critical'; });
  var hasHigh = analysis.risks.some(function(r: any) { return r.risk === 'high'; });
  var hasMedium = analysis.risks.some(function(r: any) { return r.risk === 'medium'; });
  ...
  analysis.tags.forEach(function(t: any) {
    t = t.replace('@', '');
...
function getChangedFiles(ref: any) {
  ref = ref || 'HEAD';
  try {
    ...
    if (fs.existsSync(dirPath)) {
      try {
        ...
        entries.forEach(function(e: any) {
          ...
          if (fs.statSync(fp).isFile() && ((e as string).endsWith('.js') || (e as string).endsWith('.feature'))) {
...
var SmartTestSelectorAgent = {
  ...
  selectTests: function(options: any) {
  ...
  run: function(input: any) {
    ...
    var result: any = this.selectTests(options);
    ...
    lines.push('');
    (result.analysis.features.length > 0 ? result.analysis.features : ['(none)']).forEach(function(f: any) {
      lines.push('- ' + f);
    });
    lines.push('');
    ...
    lines.push('|---|---|---|');
    result.analysis.risks.forEach(function(r: any) {
    ...
    lines.push('');
    result.selection.changedFileList.forEach(function(f: any) {
      lines.push('- ' + f);
    });
  ...
  analyzeChangedFiles: function(fileList: any) {
...
function main() {
  ...
  if (command === 'run' || command === 'select') {
    var result: any = SmartTestSelectorAgent.run({});
  ...
  if (command === 'analyze') {
    ...
    var result: any = SmartTestSelectorAgent.analyzeChangedFiles(files);
  ...
  if (command === 'reset') {
    var result: any = SmartTestSelectorAgent.resetHistory();
...
export default SmartTestSelectorAgent;
