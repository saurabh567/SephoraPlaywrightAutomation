...
function collectTestSummary() {
  ...
  const allScenarios = features.flatMap(function(f) {
    return (f.elements || []).filter(function(e) { return e.type === 'scenario'; });
  });
  const passed = allScenarios.filter(function(s) {
    return (s.steps || []).every(function(st) { return st.result && st.result.status === 'passed'; });
  }).length;
  const failed = allScenarios.filter(function(s) {
    return (s.steps || []).some(function(st) { return st.result && st.result.status === 'failed'; });
  }).length;
  const skipped = allScenarios.filter(function(s) {
    return (s.steps || []).every(function(st) {
...
function collectPipelineHistory() {
  ...
  return state.runs.map(function(r) {
    return {
      ...
      status: r.status,
      summary: r.summary || {},
      cycles: (r.cycles || []).map(function(c) {
        return {
          platform: c.platform,
...
function generateDashboardHtml(data) {
  ...
  var aiReportRows = '';
  data.aiReports.forEach(function(r) {
    var icon = r.exists ? '&#x2705;' : '&#x274C;';
  ...
  var pipelineRows = '';
  data.pipelineHistory.forEach(function(r) {
  ...
  var deviceRows = '';
  data.deviceFarmHistory.forEach(function(r) {
  ...
  data.pipelineReports.forEach(function(r) {
  ...
  return [
    ...
    '<div class="card"><h3>AI Reports</h3><div class="val">' + data.aiReports.filter(function(r) { return r.exists; }).length + '/' + data.aiReports.length + '</div><div class="sub">generated reports</div></div>',
  ...
  run: function(input) {
    ...
    return {
      ...
      summary: {
        ...
        aiReports: data.aiReports.filter(function(r) { return r.exists; }).length,
...
function main() {
  ...
  if (command === 'generate' || command === 'run') {
    var result = AIDashboardAgent.run();
