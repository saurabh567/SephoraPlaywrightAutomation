...
async function preparePR({ appliedFiles = [], backupRoot = '', proposals = [], smokeResult = 'not-run', baseBranch = null, force = false } = {}) {
  ensureReportDir();
  // Ensure appliedFiles is array of relative paths
  appliedFiles = Array.isArray(appliedFiles) ? appliedFiles : [];
  ...
  const prSummary = {
    ...
    proposalsApplied: proposals.map(p => ({ id: p.id || p, title: p.title || (p.failure && p.failure.feature) || 'n/a', risk: p.risk || 'n/a' })),
    commitMessage
  };
  ...
  if (prSummary.proposalsApplied.length) prSummary.proposalsApplied.forEach(p => lines.push(`- ${p.id}: ${p.title} (risk: ${p.risk})`)); else lines.push('- (none)');
  lines.push('');
