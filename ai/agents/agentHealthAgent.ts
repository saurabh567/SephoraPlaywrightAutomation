...
class AgentHealthAgent {
  ...
  async _generateHealthReport(healthTracker: any, registry: any) {
    const allHealth = healthTracker.getAllHealth();
    const summary = healthTracker.getSummary();
    const data = registry.getAll();
    const allKeys = new Set(data.map(function(a: any) { return a.key; }));
    ...
    var sorted = (Object.entries(allHealth) as [string, any][]).sort(function(a: any, b: any) {
      var healthOrder: Record<string, number> = { critical: 0, degraded: 1, unknown: 2, healthy: 3 };
  ...
  async _generatePerformanceReport(healthTracker: any, registry: any = {}) {
    const allHealth = healthTracker.getAllHealth();
    const summary = healthTracker.getSummary();
    ...
    var sorted = (Object.entries(allHealth) as [string, any][])
    ...
    md.push('');
    var top = (Object.entries(allHealth) as [string, any][])
    ...
    md.push('');
    var bottom = (Object.entries(allHealth) as [string, any][])
      .filter(function(e) { return e[1].totalRuns > 0; })
  ...
  async _generateLatencyReport(healthTracker: any, registry: any = {}) {
    const allHealth = healthTracker.getAllHealth();
    ...
    md.push('');
    var durations = (Object.values(allHealth) as any[]).map(function(e) { return e.lastDuration; }).filter(function(d) { return d > 0; });
