...
import AIDashboardAgent from './AIDashboardAgent';
import ConsolidatedReportAgent from './ConsolidatedReportAgent';
import reportSummarizationAgent from './reportSummarizationAgent';
...
export const run = async function run(input: any = {}) {
    ...
    if (mode === "dashboard") {
      ...
      catch (err: any) { console.warn("[ReportAgent] Dashboard strategy failed:", err.message); }
    }
    if (mode === "consolidated") {
      ...
      catch (err: any) { console.warn("[ReportAgent] Consolidated strategy failed:", err.message); }
    }
    if (mode === "summary") {
      ...
      catch (err: any) { console.warn("[ReportAgent] Summary strategy failed:", err.message); }
    }
    console.log('[ReportAgent] Generating execution report');
    const reportData = {
    ...
    if (fs.existsSync(cucumberPath)) {
      try {
        ...
        const passed = allScenarios.filter(s => (s.steps || []).every((st: any) => st.result?.status === 'passed'));
        const failed = allScenarios.filter(s => (s.steps || []).some((st: any) => st.result?.status === 'failed'));
        const skipped = allScenarios.filter(s => (s.steps || []).every((st: any) => st.result?.status === 'skipped' || st.result?.status === 'undefined'));
        ...
        reportData.duration = features.reduce((acc, f) => {
          return acc + (f.elements || []).reduce((elAcc: any, el: any) => {
            return elAcc + (el.steps || []).reduce((stAcc: any, st: any) => {
              return stAcc + (st.result?.duration || 0);
            }, 0);
          }, 0);
        }, 0);
        for (const f of failed) {
          const failedStep = (f.steps || []).find((st: any) => st.result?.status === 'failed');
    ...
    for (const [key, relPath] of (Object.entries(reportSources) as [string, any][])) {
    ...
    lines.push('|---|---|');
    for (const [key, status] of (Object.entries(reportData.sources) as [string, any][])) {
