...
module.exports = {
  run: async function run(input = {}) {
    ...
    try {
      ...
      changedFiles = output.trim().split('\n').filter(Boolean).map(function(line) { return line.replace(/^[MARC??]+\s+/, '').trim(); });
