  ...
  try {
    ...
    return keys.map(function(k) {
      ...
      return {
        ...
        methods: agent ? Object.keys(agent).filter(function(m) { return m !== 'name' && m !== 'version'; }) : [] as any[],
      };
    });
  } catch { return []; }
}
...
function generateAgentCatalog(agents: any) {
  var lines: any[] = [];
  ...
  lines.push('|---|---|---|---|---|---|');
  agents.sort(function(a: any, b: any) { return a.key.localeCompare(b.key); }).forEach(function(a: any, i: any) {
...
function generateArchitectureDoc(agents: any) {
  var lines: any[] = [];
  ...
  // Agent categories
  var categories = {
    'Generation': agents.filter(function(a: any) { return ['testCaseGeneration','featureFileGeneration','stepDefinitionGeneration','pageObjectGeneration','apiTestGeneration','mobileTestGeneration','testDataPipeline'].indexOf(a.key) !== -1; }),
    'Analysis & RCA': agents.filter(function(a: any) { return ['failureAnalysis','rootCauseAnalysis','playwrightCodeReview','reportSummarization','anomalyDetection'].indexOf(a.key) !== -1; }),
    'Healing & Pipeline': agents.filter(function(a: any) { return ['locatorHealing','selfHealingAutomation','selfHealingPipeline'].indexOf(a.key) !== -1; }),
    'Execution & Orchestration': agents.filter(function(a: any) { return ['execution','testExecution','executionMemory','planner','decision','retry','healing','rca','unifiedOrchestrator'].indexOf(a.key) !== -1; }),
    'Mobile & Device Farm': agents.filter(function(a: any) { return ['mobile','mobileTestGeneration','mobileDeviceFarm'].indexOf(a.key) !== -1; }),
    'Dashboard & Reporting': agents.filter(function(a: any) { return ['aiDashboard','report','reportSummarization','pr','prPreparation'].indexOf(a.key) !== -1; }),
    'CI/CD & Infrastructure': agents.filter(function(a: any) { return ['jenkins','jenkinsBuildFailureAnalysis','mcpHealthCheck','monitoring'].indexOf(a.key) !== -1; }),
    'Security & Quality': agents.filter(function(a: any) { return ['secretsVault','smartTestSelector','ecosystemReadiness','visual','impact','release','api'].indexOf(a.key) !== -1; }),
  };

  lines.push('## Agent Categories');
  lines.push('');
  Object.keys(categories).forEach(function(cat: any) {
    var catAgents = (categories as Record<string, any>)[cat];
    if (catAgents.length > 0) {
      lines.push('### ' + cat + ' (' + catAgents.length + ' agents)');
      lines.push('');
      catAgents.forEach(function(a: any) {
...
function generateWorkflowsDoc(agents: any) {
  var lines: any[] = [];
  ...
  agents.sort(function(a: any, b: any) { return a.key.localeCompare(b.key); }).forEach(function(a: any) {
    ...
    var filename = (agentFileMap as Record<string, any>)[a.key] || a.key + 'Agent.js';
...
function generateEcosystemReadme(agents: any) {
  var lines: any[] = [];
...
var DocumentationGeneratorAgent = {
  ...
  regenerateFile: function(docName: any) {
    var agents = getAgentList();
    var map = {
    ...
    var entry = (map as Record<string, any>)[docName];
...
function main() {
  ...
  if (command === 'generate' || command === 'all') {
    var result: any = DocumentationGeneratorAgent.generate();
  ...
  if (command === 'regenerate') {
    ...
    var result: any = DocumentationGeneratorAgent.regenerateFile(docName);
    console.log(JSON.stringify(result, null, 2));
    process.exit(result.ok ? 0 : 1);
  }
...
export default DocumentationGeneratorAgent;
