...
class ObservabilityAgent {
  ...
  _collectRegistryData(registry) {
    ...
    return {
      ...
      agents: all.map(a => ({
        ...
        conditions: (a.metadata.conditions || []).map(c => typeof c === 'string' ? c : c.type),
  ...
  _collectHealthData() {
    try {
      ...
      return { summary: { totalAgents: 0 }, agents: {} };
    }
  }
      ...
      try {
        ...
        if (fs.existsSync(fullPath)) {
          ...
          if (store.key === 'performance' && data.runs) {
            ...
            memory[store.key] = { totalEntries: data.failures.length, unresolved: data.failures.filter(f => !f.resolved).length };
    ...
    try {
      ...
      if (fs.existsSync(execPath)) {
        ...
        return {
          ...
          passed: allScenarios.filter(s => (s.steps || []).every(st => st.result?.status === 'passed')).length,
          failed: allScenarios.filter(s => (s.steps || []).some(st => st.result?.status === 'failed')).length,
          skipped: allScenarios.filter(s => (s.steps || []).every(st => st.result?.status === 'skipped' || st.result?.status === 'undefined')).length
        };
      }
    } catch (e) {}
    return {};
  ...
  _generateDashboardHtml(data) {
    ...
    const agentRows = reg.agents.sort((a, b) => a.key.localeCompare(b.key)).map(a => {
    ...
    const topoOrder = (reg.topologicalOrder || []).map((k, i) => `<div class="topo-item">${i+1}. ${k}</div>`).join('\n');

    const timelineRows = (data.events.recent || []).map(e => {
...
<div class="card"><h3>Memory Stores</h3><div class="val" style="color:#bc8cff">${Object.keys(mem).length}</div><div class="sub">${Object.values(mem).reduce((s,m) => s + (m.totalEntries||0), 0)} total entries</div></div>
...
${Object.entries(stages).map(([s, agents]) => `<tr><td><strong>${s}</strong></td><td>${agents.length}</td><td>${agents.join(', ')}</td></tr>`).join('\n')}
</table>
</div>
