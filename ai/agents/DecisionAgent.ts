...
import commandBuilder from '../core/PlaywrightCommandBuilder';
...
agent.run = async function run(input) {
  ...
  var decision: Record<string, any> = {
  ...
  try {
    ...
    engineDecisions = {
      agents: { always: [] as any[], conditional: [] as any[], skip: [] as any[] },
  ...
  var engineCtx = engineDecisions && engineDecisions.context ? engineDecisions.context : {} as Record<string, any>;
  ...
  if (reporter) {
    for (var ri = 0; ri < reporter.length; ri++) {
      ...
      if (Array.isArray(rep)) {
        ...
        catch (e: any) { extraArgs.push('--reporter', rep[0]); }
      } else {
