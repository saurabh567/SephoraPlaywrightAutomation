...
export const run = async function run(input: any = {}) {
    ...
    try {
      ...
      const responseTime: any = await new Promise((resolve: any, reject: any) => {
        const req = protocol.get(baseUrl, (res: any) => {
    ...
    if (fs.existsSync(reportPath)) {
      try {
        ...
        testResults.passed = allScenarios.filter(s => (s.steps || []).every((st: any) => st.result?.status === 'passed')).length;
        testResults.failed = allScenarios.filter(s => (s.steps || []).some((st: any) => st.result?.status === 'failed')).length;
