...
module.exports = {
  run: async function run(input = {}) {
    ...
    // 1. Pass Rate from cucumber report
    ...
    if (fs.existsSync(reportPath)) {
      try {
        ...
        passedScenarios = allScenarios.filter(s => (s.steps || []).every(st => st.result?.status === 'passed')).length;
        failedScenarios = allScenarios.filter(s => (s.steps || []).some(st => st.result?.status === 'failed')).length;
    ...
    // 3. Flakiness from retry history
    ...
    if (fs.existsSync(retryStatePath)) {
      try {
        ...
        if (history.length > 0) {
          const retriedThatPassed = history.filter(r => r.allPassed).length;
          const flakinessRate = history.length > 0 ? retriedThatPassed / history.length : 0;
          factors.flakiness.score = 1 - flakinessRate; // Lower flakiness is better
        }
      } catch (e) { /* ignore */ }
    }

    // 4. Risk assessment from impact analysis
