...
import { readCucumberSummary } from '../tools/cucumberReportReader';
import LocatorApplier from './locatorHealingApplier';
import LocatorAnalyzer from '../tools/locatorAnalyzer';
const timestamp = () => new Date().toISOString().replace(/[:.]/g,'-');

// Lazy require to avoid circular dependency with HealingAgent
let HealingAgent: any = null;
...
function readFailureInput(input: any = {}) {
...
function buildQuery(failures: any) {
...
agent.suggestWithRag = async function suggestWithRag(input: any = {}) {
...
agent.analyzeFailures = async function analyzeFailures(input: any = {}) {
  const failures = readFailureInput(input);
  if (!failures.length) return { skipped: true, reason: 'No failures found' };

  const candidates = await Promise.all(failures.map(async (f: any) => {
...
agent.applySafeFixes = async function applySafeFixes(options: any = {}) {
  ...
  const lowRisk = (data.proposals || []).filter((p: any) => p.risk === 'LOW');
  if (!lowRisk.length) return { applied: [] as any[], reason: 'No LOW-risk proposals' };
...
agent.rollback = async function rollback(backupRoot: any) {
