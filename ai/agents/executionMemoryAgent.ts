...
class ExecutionMemoryAgent {
  ...
  safeReadJson(filePath: any) {
    try {
      if (fs.existsSync(filePath)) {
        const raw = fs.readFileSync(filePath, 'utf8').trim();
        if (raw) return JSON.parse(raw);
      }
    } catch (_: any) { /* ignore corrupt files */ }
    return null;
  }
  ...
  buildEntryFromExecutionState() {
    ...
    const runs = Array.isArray(state.runs) ? state.runs : [] as any[];
...
export default new ExecutionMemoryAgent();
