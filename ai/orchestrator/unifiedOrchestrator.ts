...
import router from '../core/AgentRouter';
import bus from '../core/EventBus';
import healthTracker from '../core/AgentHealthTracker';
...
async function executeAgents(plan: any, phaseId: any, options: any) {
...
async function phase0Preflight(contextOptions: any) {
  ...
  const results: Record<string, any> = { services: {}, environment: {} };
  ...
  catch (e: any) { results.services.unifiedHealth = { ok: false, error: e.message }; console.warn('  [⚠] Health:', e.message); }

  try { const o = await ollamaManager.ensureRunning(); results.services.ollama = { ok: true }; console.log('  [✓] Ollama'); }
  catch (e: any) { results.services.ollama = { ok: false, error: e.message }; console.warn('  [⚠] Ollama:', e.message); bus.emit(bus.EVENTS.OLLAMA_FAILED, { error: e.message }); }

  try { const c = await chromaManager.ensureRunning(); results.services.chroma = { ok: true }; console.log('  [✓] Chroma'); }
  catch (e: any) { results.services.chroma = { ok: false, error: e.message }; console.warn('  [⚠] Chroma:', e.message); bus.emit(bus.EVENTS.CHROMA_FAILED, { error: e.message }); }
...
async function phase1Execution(contextOptions: any) {
  ...
  const results: Record<string, any> = { platform, exitCode: null };
  ...
  if (isMobile) {
    ...
    if (appiumEntry && typeof appiumEntry.module.startServerIfNeeded === 'function') {
      ...
      } catch (e: any) { console.warn('  [⚠] Appium:', e.message); }
    }
  }
  ...
  if (isMobile) {
    ...
    if (appiumEntry && typeof appiumEntry.module.stopServerIfStartedByFramework === 'function') {
      ...
      catch (e: any) { console.warn('  [⚠] Appium stop:', e.message); }
    }
  }
...
async function phase2AIAnalysis(execPhase: any, contextOptions: any) {
  ...
  } catch (e: any) { console.warn('  [⚠] Ingestion:', e.message); }
  ...
  } catch (e: any) { console.warn('  [⚠] Report:', e.message); }
...
async function phase3MultiAgent(execPhase: any, contextOptions: any) {
...
async function phase4Reporting(execPhase: any, contextOptions: any) {
  ...
  } catch (e: any) { console.warn('  [⚠] Dashboard:', e.message); }
...
async function canonicalizeCucumberReport() {
  function findReport(dir: any): any {
    if (!fs.existsSync(dir)) return null;
    for (const e of fs.readdirSync(dir)) {
      const full = path.join(dir, e);
      let stat; try { stat = fs.statSync(full); } catch (ex: any) { continue; }
      if (stat.isDirectory()) {
  ...
  async function waitForValidJson(filePath: any, timeout = 30000, interval = 500) {
    const start = Date.now();
    while (Date.now() - start < timeout) {
      try { const raw = fs.readFileSync(filePath, 'utf8'); if (raw.trim() && JSON.parse(raw)) return true; }
      catch (e: any) { await new Promise(r => setTimeout(r, interval)); }
    }
    return false;
  }
...
async function orchestrate(options: any = {}) {
  ...
  const orchestrationResult: Record<string, any> = {
    startedAt: timestamp(),
    completedAt: null,
    durationMs: 0,
  ...
  if (cliArgs.dryRun) {
    registry.discover().then(async () => {
      ...
      for (const a of plan.plan) {
        const condStr = (a.metadata.conditions || []).map((c: any) => typeof c === 'string' ? c : c.type).join(', ') || 'always';
