...
import os from 'os';
import { main as generateEnterpriseDashboard } from '../../utils/generateEnterpriseDashboard';
/**
...
function executePlatform(platformDef: any, results: any, timeoutMs: any) {
...
function executePlatformsParallel(platformDefs: any, results: any) {
  const startTime = Date.now();
  const nameList = platformDefs.map((p: any) => p.name).join(' + ');
  ...
  const children = platformDefs.map((platformDef: any) => {
    const result = new PlatformResult(platformDef.name);
    result.startedAt = nowISO();
    result.command = `npm run ${platformDef.script}`;
  ...
  const activeEntries = children.filter((c: any) => c.process !== null);
  const completedResults: any = [];

  function waitForProcess(entry: any) {
    return new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => {
        ...
        try { proc.kill('SIGTERM'); } catch (_: any) {}
        try { proc.kill('SIGKILL'); } catch (_: any) {}
  ...
  const promises = activeEntries.map((entry: any) => waitForProcess(entry));

  // Also include skipped results
  const skippedEntries = children.filter((c: any) => c.process === null);
  ...
  return new Promise((resolve) => {
    Promise.all(promises).then(() => {
      ...
      const androidResult = completedResults.find((r: any) => r.platform === 'ANDROID');
      const iosResult = completedResults.find((r: any) => r.platform === 'IOS');
...
function collectReportPaths(platform: any) {
  ...
  const entries = (reportMap as Record<string, any>)[platform] || [];
...
function generateDashboardData(allResults: any) {
  ...
  const totalDuration = allResults.reduce((sum: any, r: any) => sum + r.durationMs, 0);
  ...
  const data = {
    ...
    platforms: allResults.map((r: any) => ({
      name: r.platform,
      status: r.status,
      exitCode: r.exitCode,
    ...
    executionLog: allResults.map((r: any) => ({
...
function generatePipelineReport(allResults: any) {
  ...
  for (const p of data.platforms) {
    for (const [label, info] of (Object.entries(p.reportPaths) as [string, any][])) {
  ...
  data.executionLog.forEach((entry: any, i: any) => {
    const icon = entry.status === 'completed' ? '✅' : entry.status === 'skipped' ? '⏭️' : '❌';
    const phase = (phaseMap as Record<string, any>)[entry.platform] || '-';
...
async function orchestrate(options: any = {}) {
  // ── AI Agent Monitor: Emit execution lifecycle events ──
  let eventBus = null;
  try { eventBus = require("../core/EventBus"); } catch (_: any) {}
  if (eventBus) {
  ...
  try {
    ...
    if (dashResult && dashResult.htmlPath) {
      ...
      const dashPlatform = allResults.find((r: any) => r.platform === 'DASHBOARD');
      if (dashPlatform) {
  ...
  for (const r of allResults) {
    ...
    const phase = (phaseMap as Record<string, any>)[r.platform] || '-';
...
export { orchestrate, PLATFORMS, SEQUENTIAL_PLATFORMS, MOBILE_PLATFORMS, REPORTING_PLATFORMS };
export default { orchestrate, PLATFORMS, SEQUENTIAL_PLATFORMS, MOBILE_PLATFORMS, REPORTING_PLATFORMS };
