...
function readPidFile() {
  try { return fs.readJsonSync(PID_PATH); } catch (e: any) { return null; }
}
function removePidFile() { try { fs.removeSync(PID_PATH); } catch (e: any) {} }

function commandExists(cmd: any) {
  try { return spawnSync('which', [cmd], { stdio: ['ignore', 'pipe', 'ignore'] }).status === 0; } catch (e: any) { return false; }
}
...
function pullModelSync(model: any) {
...
async function stop() {
  ...
  try { process.kill(info.pid, 'SIGTERM'); } catch (e: any) { try { fs.appendFileSync(LOG_PATH, `\n[OllamaManager] kill error: ${e.message}\n`); } catch (_: any) {} }
  removePidFile();
...
export { isHealthy, start, stop, ensureRunning, pullModelSync, readPidFile };
export default { isHealthy, start, stop, ensureRunning, pullModelSync, readPidFile };
