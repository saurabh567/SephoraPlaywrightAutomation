...
class FailureMemoryStore {
  ...
  _ensure() {
    ...
    if (!fs.existsSync(STORE_PATH)) {
      fs.writeJsonSync(STORE_PATH, { failures: [], version: 1 }, { spaces: 2 });
    }
  }
   ...
   */
  recordFailures(executionSummary) {
    ...
    for (const f of failures) {
      ...
      const existing = data.failures.find(x => x.signature === signature);
   ...
   */
  markResolved(signature, resolutionRun) {
    const data = this._read();
    const failure = data.failures.find(f => f.signature === signature);
    if (failure) {
  ...
  getUnresolved() {
    const data = this._read();
    return data.failures.filter(f => !f.resolved);
  }
...
module.exports = FailureMemoryStore;
