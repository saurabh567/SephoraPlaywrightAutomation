import startupTimer from './startup-timer';
...
function isApiScenario(scenario: any) {
  if (!scenario || !scenario.pickle || !scenario.pickle.tags) return false;
  return scenario.pickle.tags.some((t: any) => {
...
function getMobileAppId() {
  ...
  const MobileSessionManager = require('../framework/mobile/MobileSessionManager').default;
...
async function validateMobilePrerequisites() {
  ...
  try {
    ...
    const healthy = await new Promise((resolve) => {
      const req = http.get(appiumUrl.origin + '/status', { timeout: 3000 }, (res: any) => {
        let data = '';
...
BeforeAll(async function () {
  ...
  if (isWebExecution) {
    const WebDriverFactory = require('../framework/web/WebDriverFactory').default;
...
Before(async function (scenario: any) {
  ...
  if (isWebExecution) {
    ...
    const BrowserCacheCleanup = require('../framework/common/BrowserCacheCleanup').default;
    await BrowserCacheCleanup.clearWeb({ page: this.page }).catch(function(err: any) {
  ...
  if (isMobileExecution) {
    // Web-mode mobile sessions (Chrome/Safari browser) have no native app
    // first-launch flow — ApplicationInitializer targets native app onboarding.
    // The Given step navigates the browser, so skip the native initializer.
    if ((isAndroidWeb() || isIOSWeb()) && !isAndroidNative()) {
      logger.info("[Hooks] Web-mode mobile session — skipping native ApplicationInitializer");
      DriverManager.markStartupCompleted();
    } else if (DriverManager.isStartupCompleted()) {
      ...
      try {
        var ApplicationInitializer = require("../framework/mobile/ApplicationInitializer").default;
  ...
  if (scenario.result.status === Status.FAILED) {
    ...
    if (this.driver) {
      try {
        ...
        const ScreenshotUtility = require('../framework/common/ScreenshotUtility').default;
        ...
        try {
          ...
          const MobileDebugUtility = require('../utils/mobileDebugUtility').default;
          await MobileDebugUtility.dumpPageState(this.driver, 'failed-scenario-' + scenario.pickle.name);
