...
class MobileAmazonCartPage extends MobileBasePage {
        ...
        for (var i = 0; i < candidates.length; i++) {
          ...
          if (
            ...
            if (rect.width > 0 && rect.height > 0) {
              ...
              try { el.click(); return { success: true, method: 'native' }; }
              catch(e) {
                try {
   ...
   */
  async _tryJsCheckout(allErrors) {
    try {
      const result = await this.driver.execute(function() {
        var selectors = [
        ...
        for (var s = 0; s < selectors.length; s++) {
          ...
          for (var i = 0; i < els.length; i++) {
            ...
            if (rect.width > 0 && rect.height > 0) {
              el.scrollIntoView({ behavior: 'instant', block: 'center' });
              try { el.click(); return { success: true, method: 'native', selector: selectors[s] }; }
              catch(e) {
                try {
                  var evt = new MouseEvent('click', { bubbles: true, cancelable: true });
                  el.dispatchEvent(evt);
        ...
        for (var i = 0; i < allElements.length; i++) {
          ...
          if (text.indexOf('proceed') !== -1 || text.indexOf('checkout') !== -1 ||
            ...
            if (rect.width > 0 && rect.height > 0) {
              el.scrollIntoView({ behavior: 'instant', block: 'center' });
              try { el.click(); return { success: true, method: 'textMatch' }; }
              catch(e) {
                try {
                  var evt = new MouseEvent('click', { bubbles: true, cancelable: true });
                  el.dispatchEvent(evt);
   ...
   */
  async _captureCartDiagnostics(action, selectors, allErrors) {
    try {
      ...
      if (elements && elements.length > 0) {
        var visible = elements.filter(function(e) { return e.visible; });
