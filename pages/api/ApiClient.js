...
class ApiError extends Error {
   ...
   */
  constructor(message, status = 0, body = null, method = '', url = '', responseTime = 0) {
    super(message);
    this.name = 'ApiError';
    this.status = status;
    this.body = body;
...
class ApiResponse {
   ...
   */
  constructor({ status, body, headers, responseTime, requestMethod, requestUrl, error } = {}) {
    this.status = status;
    this.body = body;
   ...
   * @throws {ApiError}
   */
  expectBodyField(fieldName, expected) {
