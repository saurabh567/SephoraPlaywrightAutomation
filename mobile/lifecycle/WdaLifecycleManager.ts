...
class WdaLifecycleManager {
  ...
  async build() {
    ...
    return new Promise<any>(function(this: any, resolve: any) {
      ...
      var timeout = setTimeout(function(this: any, ) {
        child.kill('SIGTERM');
        resolve({
          built: false,
      ...
      child.on('close', function(this: any, code: any) {
  ...
  async checkHealth() {
    return new Promise<any>(function(this: any, resolve: any) {
      ...
      var tryUrl = function(this: any, urlIndex: any) {
        if (urlIndex >= urls.length) {
          resolve({ running: false, healthy: false, error: 'All endpoints unreachable' });
          return;
        }

        var req = http.get(urls[urlIndex], { timeout: 3000 }, function(this: any, res) {
          var body = '';
          res.on('data', function(this: any, chunk) { body += chunk; });
          res.on('end', function(this: any, ) {
            var healthy = (res.statusCode || 0) >= 200 && (res.statusCode || 0) < 400;
            resolve({
              running: true,
              healthy: healthy,
              error: null
        ...
        req.on('error', function(this: any, err) {
          tryUrl(urlIndex + 1);
        });
  ...
  async stop() {
    ...
    if (fs.existsSync(this.pidPath)) {
      try { fs.unlinkSync(this.pidPath); } catch (_: any) {}
    }
   ...
   */
  _sleep(ms: any) {
    return new Promise(function(this: any, resolve) { setTimeout(resolve, ms); });
  }
}

export default WdaLifecycleManager;
