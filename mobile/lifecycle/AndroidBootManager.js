...
class AndroidBootManager {
   ...
   */
  static async waitForBoot(options) {
    options = options || {};
  ...
  static unlockDevice(serial)    { DeviceManager._unlockDevice(); }
  static verifyApplicationInstalled(pkg) { DeviceManager.verifyApplicationInstalled(pkg); }

  static _exec(cmd, timeout)     { return DeviceManager._exec(cmd, timeout); }
  static _sleep(ms)              { return DeviceManager._sleep(ms); }
  static _parseAdbDevices(out)   { return DeviceManager._parseAdbDevices(out); }
  static _collectDiagnostics(l)  { DeviceManager._collectDiagnostics(l); }
  static _collectLaunchDiagnostics(pkg) { DeviceManager._collectDiagnostics('launch-' + pkg); }
  static _waitForPackageForeground() { return Promise.resolve(false); }
  static _handlePreAppiumCleanup() {}
}

module.exports = AndroidBootManager;
