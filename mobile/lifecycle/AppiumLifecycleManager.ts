...
class AppiumLifecycleManager {
  ...
  async checkStatus() {
    return new Promise<any>(function(this: any, resolve: any) {
      var req = http.get(this.statusUrl, { timeout: 5000 }, function(res: any) {
        var body = '';
  ...
  _clearState() {
    for (var i = 0; i < [this.pidPath, this.statePath].length; i++) {
      ...
      if (fs.existsSync(p)) {
        try { fs.unlinkSync(p); } catch (_: any) {}
      }
    }
  }
   ...
   */
  async waitForHealthy(timeoutMs: any) {
...
export default AppiumLifecycleManager;
