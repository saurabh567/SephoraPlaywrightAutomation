...
class DeviceManager {
   ...
   */
  static async bootAndVerifyEmulator(options) {
  ...
  static async detectAndroidDevice(options) {
    ...
    return { detected: false, devices: [], message: 'No devices after ' + retries + ' attempts' };
  }
  ...
  static _killStaleProcesses() {
    ...
    try { execSync('sleep 2', { timeout: 3000 }); } catch (_) {}
  ...
  static _launchEmulator(avdName, allowSnapshots) {
  ...
  static _adbWaitForDevice(timeoutMs) {
  ...
  static async _verifyDeviceStateOrRecover(timeoutMs, pollInterval, overallStart) {
  ...
  static async _waitForBootCompleted(timeoutMs, pollInterval, overallStart) {
  ...
  static async _waitForPackageManager(timeoutMs, pollInterval) {
  ...
  static async _verifyLauncher(timeoutMs, pollInterval) {
  ...
  static verifyApplicationInstalled(appPackage) {
    if (!appPackage) {
  ...
  static _collectDiagnostics(label) {
  ...
  static _parseAdbDevices(output) {
    if (!output) return [];
