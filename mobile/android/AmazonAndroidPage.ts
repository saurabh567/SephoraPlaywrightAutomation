...
class AmazonAndroidPage extends MobileBasePage {
  ...
  async verifySearchResultsVisible(productName?: any) {
    await this.waitForSearchResultsPage(productName);
  }
  ...
  async verifyAddToCartFlowComplete(addedToCart: any) {
    if (!addedToCart) {
      await this.verifyProductDetailsVisible();
      return;
    }
  ...
  async verifyFooterLinksHaveValidUrls(footerLinks: any) {
    if (!footerLinks || footerLinks.some((link: any) => !link.href)) {
  ...
  async waitForDisplayed(locator: any, timeout = this.timeout) {
  ...
  async waitForSourceText(textOrPattern: any, timeout = this.timeout) {
    const matcher =
  ...
  async waitForSearchResultsPage(productName: any, timeout = 60000) {
    await this.driver.waitUntil(
      async () => {
        const source = await this.driver.getPageSource();
  ...
  async selectFirstSearchSuggestion(productName: any) {
  ...
  async tapByTextContains(text: any) {
    await this.tap(`android=new UiSelector().textContains("${text}")`);
  }

  async tapByCoordinates(x: any, y: any) {
    try {
...
export default AmazonAndroidPage;
