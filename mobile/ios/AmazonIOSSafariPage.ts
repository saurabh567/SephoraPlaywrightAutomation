import ConfigReader from '../../framework/common/ConfigReader';
...
class AmazonIOSSafariPage extends MobileBasePage {
   ...
   */
  async _logActionStart(actionLabel: any, opts: any = {}) {
      ...
      const notNowClicked = await this.driver.execute(function() {
        ...
        for (var i = 0; i < els.length; i++) {
          ...
          if (t === 'not now' || aria === 'not now' ||
            ...
            if ((els[i] as HTMLElement).offsetParent !== null) {  // visible check
              try { (els[i] as HTMLElement).click(); return { method: 'notNowClick', clicked: t || aria }; }
              catch (e: any) { try {
                var evt = new MouseEvent('click', { bubbles: true, cancelable: true });
                els[i].dispatchEvent(evt);
                return { method: 'notNowDispatch', clicked: t || aria };
              } catch (e2: any) { continue; }}
            }
          }
        }
        return { method: 'none' };
    ...
    if (/sign in or create account|sign in to continue/i.test(source)) {
      ...
      try {
        const result = await this.driver.execute(
          ...
          'if(els[i].offsetParent!==null){try{(els[i] as HTMLElement).click();return "clicked "+t;}catch (e: any){try{var evt=new MouseEvent(\'click\',{bubbles:true,cancelable:true});els[i].dispatchEvent(evt);return "dispatched "+t;}catch (e2: any){continue;}}}}}return "none";'
        );
  ...
  async waitForSearchResultsRendered(timeoutMs: any) {
  ...
  async verifyVisible(locatorOrSelector: any) {
    await this.dismissAuthOverlay();
      ...
      try {
        const jsResult = await this.driver.execute(function() {
          ...
          for (var i = 0; i < links.length; i++) {
            if ((links[i] as HTMLElement).offsetParent !== null) {
              try { (links[i] as HTMLElement).click(); return { method: 'jsClick', index: i }; }
              catch (e: any) {
      ...
      const jsResult = await this.driver.execute(function() {
        ...
        for (var i = 0; i < buttons.length; i++) {
          ...
          if (value.indexOf('add to cart') !== -1 || value.indexOf('buy now') !== -1 ||
            ...
            if ((buttons[i] as HTMLElement).offsetParent !== null) {
              try { (buttons[i] as HTMLElement).click(); return { method: 'jsTextClick', foundBy: value || text || aria }; }
              catch (e: any) {
                try { var evt = new MouseEvent('click', { bubbles: true, cancelable: true });
    ...
    try {
      const jsResult = await this.driver.execute(function() {
        ...
        for (var i = 0; i < deletables.length; i++) {
          if ((deletables[i] as HTMLElement).offsetParent !== null) {
            try { (deletables[i] as HTMLElement).click(); return { method: 'jsClick', foundBy: 'delete' }; }
            catch (e: any) {
              try {
                var evt = new MouseEvent('click', { bubbles: true, cancelable: true });
        ...
        for (var j = 0; j < allEls.length; j++) {
          ...
          if (t === 'delete' || v === 'delete' || aria === 'delete' ||
            ...
            if ((allEls[j] as HTMLElement).offsetParent !== null) {
              try { (allEls[j] as HTMLElement).click(); return { method: 'jsTextClick', foundBy: t || v || aria }; }
              catch (e: any) {
                try {
      ...
      const jsResult = await this.driver.execute(function() {
        ...
        for (var i = 0; i < buttons.length; i++) {
          ...
          if (
            ...
            if ((buttons[i] as HTMLElement).offsetParent !== null) {
              try { (buttons[i] as HTMLElement).click(); return { method: 'jsClick', foundBy: value || text || aria || name }; }
              catch (e: any) {
                try {
                  var evt = new MouseEvent('click', { bubbles: true, cancelable: true });
   ...
   */
  async _captureFullDiagnostics(name: any, diagnostics: any = {}) {
    ...
    try {
      consoleLogs = await this.driver.execute(() => {
        // Try to read from (window as any).__consoleStore (populated by a previous polyfill)
        // or return an empty array
        return (window as any).__consoleStore || [];
      }).catch(() => []);
    } catch (_: any) {}
    ...
    try {
      ...
      for (const [k, v] of (Object.entries(diagnostics) as [string, any][])) {
...
export default AmazonIOSSafariPage;
