...
class ChromeDriverManager {
   ...
   */
  static resolveChromeDriverVersion(chromeVersion: any) {
    if (!chromeVersion) {
  ...
  static ensureAppiumChromeDriverConfig() {
    ...
    try {
      ...
      var req = http.get('http://' + appiumHost + ':' + appiumPort + '/status', { timeout: 2000 }, function(res: any) {
        var body = '';
        res.on('data', function(chunk: any) { body += chunk; });
        res.on('end', function() {
          try {
            var json = JSON.parse(body);
            var args = json.value && json.value.argv ? json.value.argv : [] as any[];
            var hasAllowInsecure = args.some(function(a: any) {
...
export default ChromeDriverManager;
