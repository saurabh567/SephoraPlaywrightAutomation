...
function detectExecutionMode() {
  ...
  if (explicitMode) {
    ...
    if ((Object.values(EXECUTION_MODES) as any[]).indexOf(normalized) !== -1) {
      return normalized;
    }
  ...
  if (platform === 'IOS') {
    ...
    if (iosBrowserName.toLowerCase() === 'safari' || iosBrowserName.toLowerCase() === '') {
...
export { EXECUTION_MODES, detectExecutionMode, getExecutionMode, resetExecutionMode, isAndroidNative, isAndroidWeb, isAndroid, isIOSNative, isIOSWeb, isIOS, isMobile, isWeb, isApi };
export default { EXECUTION_MODES: EXECUTION_MODES, detectExecutionMode: detectExecutionMode, getExecutionMode: getExecutionMode, resetExecutionMode: resetExecutionMode, isAndroidNative: isAndroidNative, isAndroidWeb: isAndroidWeb, isAndroid: isAndroid, isIOSNative: isIOSNative, isIOSWeb: isIOSWeb, isIOS: isIOS, isMobile: isMobile, isWeb: isWeb, isApi: isApi };
