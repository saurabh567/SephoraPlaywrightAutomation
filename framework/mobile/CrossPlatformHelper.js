...
class CrossPlatformHelper {
  ...
  async scrollToElement(element, options = {}) {
    ...
    try {
      if (this._isIOS) {
        ...
        await this.driver.execute(function(el, blk, offset) {
    ...
    try {
      if (this._isIOS || forceJsClick) {
        ...
        try {
          ...
          const jsResult = await this.driver.execute(function(el) {
            try {
              el.click();
   ...
   */
  async switchToContext(targetContext, options = {}) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        const match = contexts.find(function(ctx) {
   ...
   */
  async waitForPageStabilized(options = {}) {
    ...
    try {
      await this.driver.waitUntil(async function() {
    ...
    try {
      await this.driver.waitUntil(async function() {
        var stable = await this.driver.execute(function(stableWindowMs) {
   ...
   */
  async waitForNavigation(currentUrl, options = {}) {
   ...
   * @returns {Promise<boolean>} True if click succeeded
   */
  async waitForElementThenClick(selector, options = {}) {
  ...
  async getPageReadyState() {
    ...
    try { info.url = await this.driver.getUrl(); } catch (_) {}
    try { info.title = await this.driver.getTitle(); } catch (_) {}
    try { info.context = await this.driver.getContext(); } catch (_) {}
    try {
...
module.exports = CrossPlatformHelper;
