...
function dumpPageSourceSync(driver: any, label: any) {
  try {
    ...
    if (typeof driver.getPageSource === 'function') {
      driver.getPageSource().then((source: any) => {
...
function captureScreenshotSync(driver: any, label: any) {
  try {
 ...
 */
async function findAny(driver: any, locators: any, timeoutMs = 5000) {
 ...
 */
async function findElementByTextContains(driver: any, text: any, timeoutMs = 5000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    try {
 ...
 */
async function waitForAbsence(driver: any, locators: any, timeoutMs = 8000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    let found = false;
    for (const locator of locators) {
 ...
 */
async function tapElement(driver: any, element: any, label: any) {
  if (!element) {
...
async function isLanguageScreen(driver: any) {
  const title = await findAny(driver, LANGUAGE_SCREEN_TITLE, 10000);
  return title !== null;
}

async function isSignInScreen(driver: any) {
 ...
 */
async function dismissNotificationShade(driver: any): Promise<boolean> {
 ...
 */
async function dismissSystemDialogs(driver: any) {
 ...
 */
async function dismissSystemNotificationsAndOverlays(driver: any) {
 ...
 */
async function stepSelectEnglish(driver: any) {
 ...
 */
async function stepVerifyEnglishSelected(driver: any) {
 ...
 */
async function stepTapContinue(driver: any) {
 ...
 */
async function stepWaitLanguageScreenDismissed(driver: any) {
 ...
 */
async function stepSkipSignIn(driver: any) {
 ...
 */
async function stepVerifyDashboardLoaded(driver: any) {
 ...
 */
async function handleFirstLaunchIfNeeded(driver: any) {
  if (!driver) {
...
export { handleFirstLaunchIfNeeded };
export default { handleFirstLaunchIfNeeded: handleFirstLaunchIfNeeded };
