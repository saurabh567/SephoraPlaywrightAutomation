...
class ApplicationInitializer {
   ...
   */
  static async _execute(driver: any, platform: any, locators: any, config: any) {
   ...
   */
  static async _handleLanguageScreen(driver: any, platform: any, locators: any, config: any) {
   ...
   */
  static async _handleOnboarding(driver: any, platform: any, locators: any) {
   ...
   */
  static async _handleSignInScreen(driver: any, platform: any, locators: any) {
   ...
   */
  static async _handlePermissions(driver: any, platform: any, locators: any) {
    let handled = false;
   ...
   */
  static async _handlePromotions(driver: any, platform: any, locators: any) {
    let handled = false;
   ...
   */
  static async _handleUpdates(driver: any, platform: any, locators: any) {
   ...
   */
  static async _handleLocationPermission(driver: any, platform: any, locators: any) {
   ...
   */
  static async _waitForDashboard(driver: any, platform: any, locators: any, config: any) {
   ...
   */
  static async _verifyDashboardQuick(driver: any, platform: any, locators: any) {
   ...
   */
  static async _waitForCondition(conditionFn: any, timeoutMs = 10000, pollIntervalMs = 500) {
   ...
   */
  static async _waitForStabilized(driver: any, minWaitMs = 500) {
   ...
   */
  static async _waitForScreenGone(driver: any, locators: any, timeoutMs = 8000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      let found = false;
   ...
   */
  static _detectPlatform(driver: any, platformHint: any) {
   ...
   */
  static _loadLocators(platform: any) {
    if (platform === 'ios') {
   ...
   */
  static _envBool(key: any, defaultValue: any) {
   ...
   */
  static async _isScreenShowing(driver: any, locators: any, timeoutMs = 5000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      for (const locator of locators) {
        try {
   ...
   */
  static async _findAndTap(driver: any, locators: any, timeoutMs = 5000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      for (const locator of locators) {
        try {
   ...
   */
  static async _tapByTextFallback(driver: any, platform: any, text: any) {
    try {
      ...
      if (elements && elements.length > 0) {
        for (const el of elements) {
          const displayed = await el.isDisplayed().catch(() => false);
          if (displayed) {
            try { await el.click(); return true; } catch (_: any) {}
          }
        }
      }
    } catch (_: any) {}
   ...
   */
  static async _tapAnyAlertButton(driver: any, platform: any) {
    if (platform === 'android') {
      ...
      try { await driver.execute('mobile: acceptAlert', {}); return true; } catch (_: any) {}
      try { await driver.execute('mobile: dismissAlert', {}); return true; } catch (_: any) {}
    }
    return false;
  }
   ...
   */
  static async _tapCoordinate(driver: any, x: any, y: any) {
    try {
   ...
   */
  static async _tapOutside(driver: any) {
