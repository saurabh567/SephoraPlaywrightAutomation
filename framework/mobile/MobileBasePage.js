...
class MobileBasePage {
   ...
   */
  constructor(driver, selectorMap) {
    this.driver = driver;
    this._platform = this._detectPlatform();
    this._selectorMap = selectorMap || { ios: {}, android: {}, shared: {} };
    this.timeout = 15000;
   ...
   */
  async findVisibleElement(selectors, options = {}) {
   ...
   */
  async findAndClick(key, fallbacks = [], options = {}) {
   ...
   */
  async _tryJsClick(key, allErrors) {
    try {
      const result = await this.driver.execute(function(selectorKey) {
          ...
          for (var t = 0; t < terms.length; t++) {
            ...
            if (text.indexOf(terms[t]) !== -1 || aria.indexOf(terms[t]) !== -1 ||
              ...
              if (rect.width > 0 && rect.height > 0) {
                ...
                try { el.click(); return { method: 'nativeClick', foundBy: terms[t] }; }
                catch(e) {
                  try {
                    var evt = new MouseEvent('click', { bubbles: true, cancelable: true });
                    el.dispatchEvent(evt);
                    return { method: 'dispatchedEvent', foundBy: terms[t] };
                  } catch(e2) { return { method: 'failed', error: e2.message }; }
                }
              }
            }
          }
   ...
   */
  async waitForStabilized(options) {
   ...
   */
  async waitForNav(currentUrl, options) {
  ...
  async _captureDiagnostics(key, selectors, allErrors) {
    try {
  ...
  async logStep(actionLabel, opts) { return this.debug.logStep(actionLabel, opts); }
  logStepEnd(actionLabel, startTime, success) { this.debug.logStepEnd(actionLabel, startTime, success); }
  async dumpPageState(label) { return this.debug.dumpPageState(label); }

  // ═══════════════════════════════════════════════════════════════════════
  // Legacy Convenience Methods
  // ═══════════════════════════════════════════════════════════════════════

  async waitForSourceText(textOrPattern, timeout) {
  ...
  async isDisplayed(key, fallbacks = []) {
    const selectors = this.getSelectors(key, fallbacks);
  ...
  async pause(ms) { await this.driver.pause(ms); }
  async getPageSource() { return this.driver.getPageSource(); }
  async getUrl() { return this.driver.getUrl().catch(() => ''); }
  async url(target) { await this.driver.url(target); }
  async execute(fn, ...args) { return this.driver.execute(fn, ...args); }
