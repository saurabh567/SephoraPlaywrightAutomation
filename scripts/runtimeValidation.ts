...
import router from '../ai/core/AgentRouter';
/**
...
async function main() {
  ...
  const allTraces: Record<string, any> = {};
  ...
  for (const ctx of contexts) {
    ...
    for (const entry of plan.plan) {
      ...
      if (!agentExecutions[agentKey]) {
        agentExecutions[agentKey] = { executions: [] as any[], totalDuration: 0, successCount: 0, failureCount: 0 };
      }
      ...
      try {
        ...
        if (typeof agentModule.run === 'function') {
          ...
          trace.status = 'noop';
          trace.output = 'Agent has no run() method (utility module)' as any;
          agentExecutions[agentKey].executions.push(trace);
          continue;
        }

        trace.status = 'success';
        trace.output = result as any;
    ...
    for (const skip of plan.skipped) {
      if (!agentExecutions[skip.key]) {
        agentExecutions[skip.key] = { executions: [] as any[], totalDuration: 0, successCount: 0, failureCount: 0 };
      }
  ...
  for (const [ctxName, traces] of (Object.entries(allTraces) as [string, any][])) {
    for (const [agentKey, trace] of (Object.entries(traces) as [string, any][])) {
  ...
  for (const [key, data] of (Object.entries(agentExecutions) as [string, any][])) {
...
function generateExecutionMatrix(allTraces: any, allPlans: any, agentExecutions: any, executedAgents: any, neverExecuted: any, utilization: any, registeredAgents: any) {
  ...
  md += '|---|---|---|---|---|---|\n';
  for (const [ctxName, plan] of (Object.entries(allPlans) as [string, any][])) {
    const traces = allTraces[ctxName] || {};
    const success = (Object.values(traces) as any[]).filter(t => t.status === 'success').length;
    const failed = (Object.values(traces) as any[]).filter(t => t.status === 'failed').length;
  ...
  for (const agent of sorted) {
    ...
    const contexts = new Set(exec.executions.map((e: any) => e.context).filter(Boolean));
...
function generateRuntimeDepGraph(allTraces: any, allPlans: any, agentExecutions: any, cycles: any) {
  ...
  md += '|---|---|\n';
  for (const [agentKey, trace] of (Object.entries(webFailures) as [string, any][])) {
