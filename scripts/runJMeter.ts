...
function getArg(flag: any, defaultValue: any) {
  const idx = args.indexOf(flag);
  if (idx !== -1 && idx + 1 < args.length) return args[idx + 1];
  return defaultValue;
}
function hasFlag(flag: any) {
...
function buildCommand(jmeterBin: any, planPath: any, jtlPath: any) {
...
function buildGuiCommand(jmeterBin: any, planPath: any) {
  const heap = config['jmeter.heap'] || '-Xms1g -Xmx2g';
  const locale = config['jmeter.locale'] || '-Duser.language=en -Duser.region=IN';
...
function parseJtlSummary(jtlPath: any) {
  ...
  const labels = (Object.entries(byLabel) as [string, any][]).map(([name, data]) => ({
    label: name,
    samples: data.count,
    errors: data.errors,
...
// --- Write summary files ---
function writeSummary(summary: any, planName: any, thresholds: any) {
