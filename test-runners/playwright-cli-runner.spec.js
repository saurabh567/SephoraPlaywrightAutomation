...
function parseFeatureFile(filePath) {
  ...
  const feature = { name: '', tags: [], scenarios: [] };
 ...
 */
function matchesTags(scenarioTags, filter) {
  if (!filter) return true;
  const filterTags = filter.split(',').map(t => t.trim());
  return filterTags.every(ft => scenarioTags.includes(ft));
}

/**
 * Check if scenario matches the platform.
 */
function matchesPlatform(scenarioTags, platform) {
  if (platform === 'WEB') return scenarioTags.includes('@web') || !scenarioTags.some(t => ['@android', '@ios', '@api'].includes(t));
...
for (const feature of features) {
  for (const scenario of feature.scenarios) {
    ...
    test(`${feature.name} > ${testName}`, async ({ page, context, browser }) => {
      ...
      global.__PLAYWRIGHT_PAGE__ = page;
      global.__PLAYWRIGHT_CONTEXT__ = context;
      global.__PLAYWRIGHT_BROWSER__ = browser;
