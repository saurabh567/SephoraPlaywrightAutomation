...
let MobileDebugUtility: any = null;
  ...
  if (isMobile(this)) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(elements.length, 30); i++) {
        const el = elements[i];
        const tag = await el.evaluate((el: any) => el.tagName).catch(() => '?');
  ...
  if (isMobile(this)) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(elements.length, 5); i++) {
        const html = await elements[i].evaluate((el: any) => el.outerHTML.substring(0, 500));
        logger.info(`  [${i}] ${html}`);
      }
    } catch (err: any) {
