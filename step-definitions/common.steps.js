...
Given('I launch the Amazon application', async function () {
  ...
  if (mobilePage) {
    await mobilePage.openHomePage();
    return skipIfBotOrCaptchaPage(mobilePage);
  }
...
Given('I search for product from test data on Amazon', async function () {
  ...
  if (mobilePage) {
    await mobilePage.openHomePage();
    const skipResult = await skipIfBotOrCaptchaPage(mobilePage);
    if (skipResult) return skipResult;
    await mobilePage.searchProduct(products.searchTerm);
    return;
  }
...
Given('I open the first Amazon product from search results', async function () {
  ...
  if (mobilePage) {
    ...
    if (skipResult) return skipResult;
    await mobilePage.searchProduct(products.searchTerm);
    await mobilePage.openFirstProductFromResults();
    return;
  }
...
When('I add the Amazon product to cart if possible', async function () {
  const mobilePage = getAmazonMobilePage(this);
  if (mobilePage) {
    this.addedToCart = await mobilePage.addToCartIfAvailable();
    return;
  }
...
Then('the page URL should contain {string}', async function (urlPart) {
  if (this.driver) {
    const mobilePage = getAmazonMobilePage(this);
    await mobilePage.waitForSourceText(urlPart, ConfigReader.get('timeout'));
    return;
  }
...
Then('the page title should contain {string}', async function (titlePart) {
  if (this.driver) {
    const mobilePage = getAmazonMobilePage(this);
    await mobilePage.waitForSourceText(titlePart, ConfigReader.get('timeout'));
    return;
  }
...
Then('I should see text {string}', async function (text) {
  if (this.driver) {
    const mobilePage = getAmazonMobilePage(this);
    await mobilePage.waitForSourceText(text, ConfigReader.get('timeout'));
    return;
  }
