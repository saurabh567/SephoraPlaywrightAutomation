 ...
 */
function resolveLanguageCode(language: any) {
  ...
  for (const [key, code] of (Object.entries(LANGUAGE_CODES) as [string, any][])) {
 ...
 */
async function isOnPreferencesPage(page: any) {
  const url = page.url();
 ...
 */
async function isOnHomePage(page: any) {
  const url = page.url();
 ...
 */
async function settle(page: any, ms: any = 2500) {
...
async function strategySelectOnPreferencesPage(page: any, languageCode: any) {
  ...
  if (!saveVisible) {
    ...
    const formSubmitted = await page.evaluate((): any => {
      const form = document.querySelector('form[action*="customer-preferences"]');
      if (form) { (form as HTMLFormElement).submit(); return true; }
      return false;
    }).catch(() => false);
...
async function strategyClickNavLanguageLink(page: any, languageCode: any) {
...
async function strategyDirectNavigate(page: any, languageCode: any) {
...
async function strategyTextMatch(page: any, language: any) {
...
async function strategyFooterSelector(page: any, language: any) {
...
async function selectLanguage(page: any, language: any) {
