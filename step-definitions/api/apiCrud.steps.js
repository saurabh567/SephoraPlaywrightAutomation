...
const ApiClient = require('../../pages/api/ApiClient');
...
// Helpers
// ---------------------------------------------------------------------------
function recordResult(scenarioName, method, url, status, responseTime, body, error) {
  apiResults.push({
...
// Report writers
// ---------------------------------------------------------------------------
function writeApiSummary(results, startTime) {
  ...
  const avgResponseTime = total > 0
    ? (results.reduce((sum, r) => sum + r.responseTime, 0) / total).toFixed(0)
    : '0';
  const fastest = total > 0 ? Math.min(...results.map(r => r.responseTime)) : 0;
  const slowest = total > 0 ? Math.max(...results.map(r => r.responseTime)) : 0;
...
async function runAiAnalysis(results) {
  ...
  const failureAnalysis = failedApis.map(r => ({
    scenario: r.scenario,
    method: r.method,
    url: r.url,
    status: r.status,
  ...
  const slowApis = results.filter(r => r.responseTime > slowThreshold);
